...
def get_calendar_events(account: str, ids: list[str]) -> list[dict]:
    ...
    for event in service.get(ids):
        # //// Neoffice — skip degenerate events. Stalwart occasionally returns
        # //// objects with no uid/id/calendarIds (seen on wide expanded-recurrence
        # //// windows); format_calendar_event then dies on the missing key and the
        # //// whole fetch fails, so the calendar renders empty. One malformed event
        # //// must not cost the user the rest of the window. ////
