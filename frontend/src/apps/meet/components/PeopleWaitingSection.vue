<template>
  <div v-if="lobbyUsers.length > 0" class="">
    <div
      class="px-4 py-2 text-xs-medium text-ink-gray-5 tracking-wide bg-surface-gray-1 flex items-center justify-between"
    >
      <span>Waiting to join</span>
      <Button variant="ghost" size="sm" @click="$emit('approve-all')" class="text-xs-medium ">
        Admit all
      </Button>
    </div>
    <div
    ...
    >
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <Badge v-if="lobbyUser.isGuest" size="sm"> Guest </Badge>
        </div>
      </div>
      <div class="flex items-center gap-2">
        <Button
