...
const toolbarHeight = computed(() => (isVisible.value ? TOOLBAR_VISIBLE_HEIGHT : '0px'))
...
const resetHideTimer = (force = false) => {
  ...
  if (!force && (isDropdownOpen.value || isHovering.value || props.isReactionPickerOpen)) {
    return
  }
...
const handleShortcut = event => {
  if ((event.ctrlKey || event.metaKey) && ['d', 'e'].includes(event.key.toLowerCase())) {
    showControls()
  }
}
