<template>
  <SettingsLayoutBase :description="'Select your preferred camera, microphone, and speaker'">
      ...
      <div v-else class="space-y-6">
        ...
        <div class="space-y-2 flex gap-4 items-center">
          ...
          <div v-if="selectedMicIdLocal" class="w-5">
            <AudioIndicator
              class="mt-2"
              :device-id="getDeviceId(selectedMicIdLocal)"
              :is-active="true"
              :sensitivity="2"
              :max-height="40"
...
const selectedCameraIdLocal = ref<string | AutocompleteOption>(selectedCameraId.value)
const selectedMicIdLocal = ref<string | AutocompleteOption>(selectedMicId.value)
const selectedSpeakerIdLocal = ref<string | AutocompleteOption>(selectedSpeakerId.value)
...
const getDeviceId = (value: string | AutocompleteOption | undefined): string => {
...
watch(selectedCameraIdLocal, newDeviceId => {
  ...
  const deviceId = typeof newDeviceId === 'object' ? newDeviceId?.value : newDeviceId
...
const handleDeviceChange = () => {
  ...
  const newCameras = cameraOptions.value.filter(
    cam => !oldCameraOptions.some(oldCam => oldCam.deviceId === cam.deviceId)
  )
  ...
  const newSpeakers = speakerOptions.value.filter(
    speaker => !oldSpeakerOptions.some(oldSpeaker => oldSpeaker.deviceId === speaker.deviceId)
  )
  ...
  if (
    ...
    !deviceManager.findDeviceById(getDeviceId(selectedCameraIdLocal.value), 'camera')
  ) {
    selectedCameraIdLocal.value = ''
  }
  if (
    selectedMicIdLocal.value &&
    !deviceManager.findDeviceById(getDeviceId(selectedMicIdLocal.value), 'microphone')
  ) {
    selectedMicIdLocal.value = ''
  }
  if (
    selectedSpeakerIdLocal.value &&
    !deviceManager.findDeviceById(getDeviceId(selectedSpeakerIdLocal.value), 'speaker')
  ) {
    selectedSpeakerIdLocal.value = ''
  }
