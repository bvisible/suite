...
const setRemoteVideoRef =
  inject<(participantId: string, el: HTMLVideoElement | null) => void>('setRemoteVideoRef')
const setScreenShareVideoRef =
  inject<(el: HTMLVideoElement | null) => void>('setScreenShareVideoRef')
const getParticipantName =
  inject<(participantId: string) => string>('getParticipantName') || (() => 'Unknown')
...
const participants = computed(() => meetingCtx.participantStore.participants) as ComputedRef<
...
const activeSpeakerIds = computed(() => meetingCtx.participantStore.activeSpeakerIds)
const pinnedTiles = computed(() => meetingCtx.gridLayout.pinnedTiles.value)
const displayScreenShares = computed(() => meetingCtx.gridLayout.displayScreenShares.value)
...
const getScreenShareTileBindings = (shareTile: {
  pinId: string
  participant: Record<string, unknown> | { user_id: string; user_name: string; avatar: string }
}) => {
  ...
  return {
    ...
    style: isPinned ? pinnedTileStyles.value[`screenshare-${shareTile.pinId}`] : tileStyle.value,
...
const getParticipantTileBindings = (participant: Participant & { isVisible: boolean }) => {
...
const localParticipant = computed(() => {
  ...
  const user_id =
    typeof userIdRaw === 'string' ? userIdRaw : userIdRaw == null ? 'local' : String(userIdRaw)
  const displayNameRaw = fullNameRaw || nameRaw || 'You'
  const user_name = typeof displayNameRaw === 'string' ? displayNameRaw : String(displayNameRaw)
  const avatar =
    typeof avatarRaw === 'string' ? avatarRaw : avatarRaw == null ? '' : String(avatarRaw)
...
const {
  ...
  pinnedTiles,
  {
    ...
    activeSpeakerIds: computed(() => meetingCtx.participantStore.activeSpeakerIds),
    stableSpeakerIds: computed(() => meetingCtx.participantStore.stableSpeakerIds),
  },
  extraTileCount
)
...
const floatingReactions = computed(() => {
  ...
  const currentUserId = (currentUser.value as Record<string, unknown> | null)?.user_id as
    | string
    | undefined
  ...
  for (const [userId, reaction] of Object.entries(reactions)) {
    ...
    const participant = userId === currentUserId ? currentUser.value : participants.value[userId]
