  ...
  try {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < numBars; i++) {
        ...
        const target = (amplified * centerBias * organic + 0.01) * props.sensitivity

        // Apply exponential smoothing to make it less jittery
        const responsiveSmoothing = 0.62 // lower -> more responsive to quiet sounds
        smoothedLevels[i] =
          smoothedLevels[i] * responsiveSmoothing + target * (1 - responsiveSmoothing)

        const displayHeight = Math.max(4, Math.min(maxHeight, smoothedLevels[i] * maxHeight))
