...
export function useMeetingPreviewPresence(meetingId: string) {
  ...
  const connectToSFU = (tokenData: PresenceTokenResponse) => {
      ...
      currentSocket.emit(
        ...
        (joinResponse: PresenceJoinResponse) => {
          if (!joinResponse.success) {
            console.error('Failed to join room for presence preview:', joinResponse.error)
          }
        }
      )

      currentSocket.emit('get_room_participants', {}, (response: PresenceParticipantsResponse) => {
    ...
    currentSocket.on('participant_joined', (data: ParticipantJoinedEvent) => {
      ...
      const existingIndex = participants.value.findIndex(p => p.user_id === newParticipant.user_id)
      if (existingIndex === -1) {
    ...
    currentSocket.on('participant_left', (data: ParticipantLeftEvent) => {
      ...
      const index = participants.value.findIndex(p => p.user_id === data.participantId)
      if (index !== -1) {
