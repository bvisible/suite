...
async function encryptChatMessage(key: CryptoKey, text: string): Promise<string> {
...
async function decryptChatMessage(key: CryptoKey, encryptedMessage: string): Promise<string> {
  ...
  const setupChatEvents = (notificationQueue: unknown) => {
    sfuClient.on('chat:message', async (data: Record<string, unknown>) => {
      ...
      if (isEncryptedChatMessage(plaintext)) {
        ...
        if (!key) {
          console.warn('E2EE chat: received encrypted message but no meeting context set')
      ...
      if (!chatStore.isChatOpen && data.fromUser !== currentUser.currentUser.value?.user_id) {
        chatStore.hasUnreadMessages = true

        ;(notificationQueue as { addNotification?: (n: unknown) => void })?.addNotification?.({
          message: plaintext,
