...
async function encryptPollPayload(
  key: CryptoKey | null,
  question: string,
  options: { text: string }[]
) {
...
export function usePoll(deps: {
  ...
  const syncEncryptedPolls = async (key: CryptoKey) => {
    ...
    const plaintextPolls = pollStore.activePolls.filter(poll => !hasEncryptedPollText(poll))
    await Promise.all(
  ...
  const fetchExistingPolls = async () => {
    ...
    try {
      ...
      if (response.success && response.polls) {
        ...
        const decrypted = (
          await Promise.all(response.polls.map(p => decryptPollPayload(key, p)))
  ...
  const setupPollEvents = (notificationQueue?: unknown) => {
    ...
    sfuClient.on('existing_polls', async (data: unknown) => {
      ...
      const decrypted = (
        await Promise.all(payload.polls.map(p => decryptPollPayload(key, p)))
      ).filter((poll): poll is PollPayloadFE => poll !== null)
