...
function getBackgroundEffectsFromStorage() {
  ...
  const selectedImage = localStorage.getItem('backgroundEffects.imageName') || ''
...
type ScreenShareStopReason = 'user-click' | 'track-ended' | 'publish-failed' | 'cleanup'

function getMediaHandler(manager: SFUMeetingManager | null): MediaHandlerLike | null {
...
export function useMediaControls(deps: MediaControlsDeps): MediaControlsAPI {
  ...
  const replacePublishedVideoTrack = async (
    ...
    if (!track) {
      console.warn(`Skipped video track swap (${reason}) - no video track available`)
      return
    }
    if (track.readyState === 'ended') {
      console.warn(`Skipped video track swap (${reason}) - track already ended`)
      return
    }
  ...
  const applyBackgroundEffectsToLocalStream = async (forceRestart = false) => {
    ...
    const hasLiveVideoTrack =
      !!localStream && localStream.getVideoTracks().some(track => track.readyState === 'live')
    ...
    try {
      ...
      const result = await backgroundEffects.applyBackgroundEffects(localStream, {
        blurIntensity: bgEffects.blurIntensity,
        backgroundBlurEnabled: bgEffects.blurEnabled,
        backgroundImageEnabled: bgEffects.imageEnabled,
        selectedBackgroundImage: bgEffects.selectedImage,
      ...
      console.warn('Failed to apply background effects to local stream:', error)
  ...
  const switchMic = async (deviceId: string) => {
    ...
    if (mh?.audioProducer && typeof mh.audioProducer.replaceTrack === 'function') {
      await mh.audioProducer.replaceTrack({ track: trackToPublish })
      return
    }

    if (!mh?.audioProducer && sfuManager.value?.transportManager) {
      const producer = await sfuManager.value.transportManager.createProducer(trackToPublish, {
        type: 'microphone',
      })
  ...
  const switchCam = async (deviceId: string) => {
    ...
    const bgEffects = getBackgroundEffectsFromStorage()
    if (bgEffects.anyEnabled || shouldApplyBackgroundEffectsWhenVideoAvailable) {
    ...
    } else if (trackToPublish && !mh?.videoProducer && sfuManager.value?.transportManager) {
      const producer = await sfuManager.value.transportManager.createProducer(trackToPublish, {
        type: 'camera',
      })
  ...
  const getProcessedAudioTrack = async (stream: MediaStream) => {
    ...
    try {
      ...
      const result = await noiseCancellation.applyNoiseCancellation(audioStream)
  ...
  const getValidDeviceId = async (storedDeviceId: string | null, deviceType: DeviceType) => {
    ...
    try {
      ...
      console.warn(`Could not validate ${deviceType} device availability:`, error)
      return storedDeviceId
    }
  }

  const buildMediaConstraints = async (videoEnabled: boolean, audioEnabled: boolean) => {
    ...
    if (videoEnabled) {
      ...
      const validCameraId = await getValidDeviceId(selectedCameraId.value, 'camera')
      if (validCameraId) {
    ...
    if (audioEnabled) {
      const validMicId = await getValidDeviceId(selectedMicId.value, 'microphone')
  ...
  const acquireUserMedia = async (
    ...
    if (videoEnabled && Object.hasOwn(deviceOverrides, 'cameraDeviceId')) {
      const validCameraId = await getValidDeviceId(deviceOverrides.cameraDeviceId, 'camera')
    ...
    if (audioEnabled && Object.hasOwn(deviceOverrides, 'micDeviceId')) {
      const validMicId = await getValidDeviceId(deviceOverrides.micDeviceId, 'microphone')
  ...
  const applySpeakerDevice = async () => {
    try {
      const validSpeakerId = await getValidDeviceId(selectedSpeakerId.value, 'speaker')
  ...
  const initializeCamera = async () => {
    try {
      ...
      if (mediaState.isCameraOn || mediaState.isMicOn) {
        const { stream } = await acquireUserMedia(mediaState.isCameraOn, mediaState.isMicOn)
  ...
  const toggleMicrophone = async () => {
    try {
      ...
      if (enable) {
        if (!stream) {
          try {
            const { stream: nextStream } = await acquireUserMedia(mediaState.isCameraOn, enable)
            stream = nextStream
            mediaState.localStream = stream
            mediaState.cameraPermissionGranted = true
            mediaState.microphonePermissionGranted = true
        ...
        if (mh?.audioProducer) {
          ...
          const producer = await sfuManager.value.transportManager.createProducer(track, {
            type: 'microphone',
          })
          mh?.setProducers({ audioProducer: producer as ProducerLike })
        }
      ...
      if (enable && currentUserId && raiseHandStore.raisedHands?.[currentUserId]) {
        try {
  ...
  const toggleCamera = async () => {
    try {
      ...
      if (enable) {
        if (!stream) {
          try {
            const { stream: nextStream } = await acquireUserMedia(true, mediaState.isMicOn)
            stream = nextStream
            mediaState.localStream = stream
            mediaState.cameraPermissionGranted = true
            if (mediaState.isMicOn) {
            ...
            try {
              ...
              if (newTrack) {
                ...
                if (mediaState.localVideo) {
                  const localVideoEl = mediaState.localVideo as HTMLVideoElement
        ...
        if (mh?.videoProducer) {
          ...
          const trackToReplace = mediaState.processedStream
            ...
            : track
          try {
            if (trackToReplace && typeof videoProducer.replaceTrack === 'function') {
  ...
  const toggleScreenShare = async () => {
    try {
      if (mediaState.isScreenSharing) {
        ...
        const hasOngoingRemoteShare = (mediaState.activeScreenShareConsumers || []).length > 0
        ...
        const screenStream = await navigator.mediaDevices.getDisplayMedia(screenShareOptions)
        if (!screenStream) throw new Error('Failed to obtain screen share stream')
        ...
        try {
          ...
          const producer = await sfuManager.value.transportManager.createProducer(screenTrack, {
            type: 'screen',
          })
          const mh = getMediaHandler(sfuManager.value)
          if (mh) {
          ...
          if (mh?.audioProducer?.paused) {
            ...
            if (micTrack && sfuManager.value?.transportManager) {
              try {
                const newProducer = await sfuManager.value.transportManager.createProducer(
                  micTrack,
                  { type: 'microphone' }
                )
                mh?.setProducers({
                ...
                console.warn('Failed to create audio producer after starting screen share', err)
              }
            }
          }
        } catch (pubErr) {
  ...
  const setScreenShareVideoRef = (el: HTMLElement) => {
    ...
    const participantId = (el as HTMLElement & { dataset: Record<string, string> }).dataset
      ?.participantId
    if (participantId) {
      ...
      if (stream instanceof MediaStream) {
        ...
        const existingStreamId = srcObject instanceof MediaStream ? srcObject.id : undefined

        if (!(el as HTMLVideoElement).srcObject || existingStreamId !== currentStreamId) {
  ...
  watch(
    ...
    async newStream => {
      const reason = newStream ? 'processed-stream-change' : 'processed-stream-removed'
  ...
  watch(noiseCancellation.error, message => {
    if (message) {
      toast.warning(`Noise cancellation unavailable: ${message}. Falling back to raw microphone.`)
    }
  })
