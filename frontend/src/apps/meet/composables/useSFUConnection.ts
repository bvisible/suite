...
import { defineAsyncComponent, h, onUnmounted, type Ref, shallowRef } from 'vue'
...
interface SFUConnectionAPI {
  ...
  handleGuestJoinResult: (joinResult: Record<string, unknown>, guestName: string) => Promise<void>
...
export function useSFUConnection(deps: {
  ...
  const handleParticipantJoined = (participant: Record<string, unknown>) => {
    ...
    audioNotificationManager.playJoinNotification(participant.participantId as string)

    const LucideUserIcon = defineAsyncComponent(() => import('~icons/lucide/user'))
    ...
    return {
      ...
      onActiveSpeakerChanged: (participantIds: string[]) => {
        ...
        const checkStability = () => {
          ...
          const currentStable = new Set(participantStore.stableSpeakerIds || [])
  ...
  const setupSFUConnection = async (
    ...
    try {
      ...
      if (!guestName) {
        ...
        const effectiveIsCohost = sfuClient.connectionDetails.isCohost || isCohost
  ...
  const setupGuestApprovalListener = (guestName: string) => {
    ...
    async function handleGuestApproved(data: Record<string, unknown>) {
      ...
      try {
        const resolvedGuestName = guestName || sessionStorage.getItem('guest_name') || 'Guest'
        ...
        if (
          ...
          if ((response as Record<string, unknown>).host_only_chat !== undefined) {
            chatStore.hostOnlyChat = !!(response as Record<string, unknown>).host_only_chat
          }
          ...
          console.error('Failed to get connection details after approval:', response)
          connectionState.connectionError = 'Failed to get authorization token after approval'
        }
      } catch (error) {
        console.error('Error fetching connection details after approval:', error)
  ...
  const handleGuestJoinResult = async (joinResult: Record<string, unknown>, guestName: string) => {
    if (!guestName || !joinResult?.guest_id) {
      connectionState.connectionError = 'Guest session not found. Please try joining again.'
      return
    }
    ...
    try {
      ...
      connectionState.guestAuthToken = (joinResult.auth_token as string) || null
