...
export interface E2EEConnectionHandshake {
  handleMeetingE2EEEnabled: (data: { meeting_id?: string; e2ee_enabled?: boolean }) => Promise<void>
...
export function useE2EEConnectionHandshake(
  ...
  controller.onHandshakeComplete = detail => {
    controller.setMeetingContext(detail.meetingSecret, detail.keyVersion, detail.signingPrivateKey)
  }
  ...
  function setupRealtimeEventListeners(): void {
    ...
    document.addEventListener('meet:e2ee-host-enabled', handleHostE2EEKeySetBound)
    document.addEventListener('meet:e2ee-needs-key-resync', handleE2EEResyncBound)
  ...
  function teardownRealtimeEventListeners(): void {
    ...
    document.removeEventListener('meet:e2ee-host-enabled', handleHostE2EEKeySetBound)
    document.removeEventListener('meet:e2ee-needs-key-resync', handleE2EEResyncBound)
  ...
  const boundHandleMeetingE2EEEnabled = controller.handleMeetingE2EEEnabled.bind(controller)
