...
export function useBackgroundEffects(): UseBackgroundEffectsReturn {
  ...
  async function loadModel(): Promise<SelfieSegmentation> {
    try {
      ...
      toast.error('Failed to load the background effects model. Please try again.')
      throw err
    }
  }

  async function loadBackgroundImage(imageUrl: string): Promise<HTMLImageElement> {
  ...
  async function applyBackgroundEffects(
    ...
    const shouldContinueProcessing = (
      ...
      return isProcessing.value && sessionId === activeSessionId && model === selfieSegmentation
    }
    ...
    try {
      ...
      if (!webglManager) {
        try {
          ...
          toast.warning('WebGL is not available. Background blur effects will be disabled.')
          webglManager = null
        }
      }
      ...
      const loadBackgroundImageData = async (selectedKey: string): Promise<void> => {
        let bgImage: BackgroundImage | null = null
        const predefinedImages = availableBackgroundImages
        bgImage = predefinedImages.find(img => img.name === selectedKey) || null

        if (!bgImage) {
          const { customBackgroundImages } = await import('../data/backgroundEffects')
        ...
        backgroundImageData = getBackgroundImageData(img, canvas.width, canvas.height)
      ...
      const useOffscreenCanvas =
        typeof OffscreenCanvas !== 'undefined' && 'MediaStreamTrackGenerator' in window
        ...
        if (settings.backgroundBlurEnabled) {
          if (webglManager) {
            try {
              ...
              if (error instanceof CompositingError && error.code === 'WEBGL_UNAVAILABLE') {
                toast.error(
              ...
              } else if (error instanceof CompositingError && error.code === 'WEBGL_BLUR_FAILED') {
                toast.error(
          ...
          if (webglManager) {
            try {
              const resultCanvas = applyVirtualBackground(canvas, maskBitmap, backgroundImageData, {
                webglManager,
              })
              outputCtx.drawImage(resultCanvas, 0, 0)
            } catch (error) {
              console.warn('Virtual background WebGL failed, disabling:', error)
              backgroundImageData = null
              outputCtx.drawImage(canvas, 0, 0)
            }
          } else {
            ...
            try {
              ...
              if (isFatalError) {
                try {
                  ...
                  console.error('Failed to recover from frame error:', recoveryError)
      ...
      const updateOptions = async (updatedOptions: BackgroundEffectOptions = {}): Promise<void> => {
        if (!isProcessing.value) {
          return
        }
        ...
        if (
          ...
          try {
            ...
            toast.error('Failed to update the selected background image. Reverting to original.')
  ...
  async function haltProcessing(options: HaltProcessingOptions = {}): Promise<void> {
