...
export function useMeetingHandlers(deps: MeetingHandlersDeps) {
  ...
  const handleGuestJoinComplete = async ({
    ...
    const guestId = (joinResult?.guest_id as string) || (deps.connectionState.guestId as string)
    ...
    await deps.sfuConnection.handleGuestJoinResult(joinResult, resolvedGuestName || '')
  }
  ...
  const tryJoinAgain = async () => {
    ...
    const isGuestSession =
      !deps.currentUser.currentUser.value?.user_id && !deps.connectionState.guestAuthToken
    if (isGuestSession) {
      deps.connectionState.isInPreview = true
      return
    }
  ...
  const handleMuteParticipant = async (participantId: string) => {
    try {
      if (deps.sfuConnection.sfuManager.value?.sfuClient) {
        deps.sfuConnection.sfuManager.value.sfuClient.sendEvent('host_control', {
    ...
    try {
      if (ban) {
        await deps.meetingDoc.setValue.submit({
          banned_users: [...(deps.meetingDoc.doc?.banned_users || []), { user: participantId }],
        })
      }
  ...
  const handleReportProblem = async () => {
    const { openProblemReportEmail } = await import('../utils/diagnostics/problemReport')
    await openProblemReportEmail({
      ...
      transportManager: deps.sfuConnection.sfuManager.value?.transportManager || null,
