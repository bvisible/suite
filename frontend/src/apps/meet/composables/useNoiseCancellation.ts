...
interface UseNoiseCancellationReturn {
  ...
  applyNoiseCancellation: (inputStream: MediaStream) => Promise<NoiseCancellationResult>
  stopProcessing: () => void
}
...
export function useNoiseCancellation(): UseNoiseCancellationReturn {
  ...
  async function preloadWorklet(): Promise<PreloadedWorklet | null> {
    ...
    preloadPromise = (async () => {
      try {
        ...
        error.value = err instanceof Error ? err.message : 'Failed to initialize noise cancellation'
        return null
      } finally {
        preloadPromise = null
      }
  ...
  function crossfadeTo(rawGain: GainNode, denoiseGain: GainNode, useDenoise: boolean): void {
  ...
  async function applyNoiseCancellation(
    ...
    try {
      ...
      sessionDenoiseSource = sessionContext.createMediaStreamSource(inputStream)
      ...
      error.value = err instanceof Error ? err.message : 'Failed to apply noise cancellation'
      isProcessing.value = false
      stopProcessing()
      return { stream: inputStream, cleanup: () => {} }
    }
