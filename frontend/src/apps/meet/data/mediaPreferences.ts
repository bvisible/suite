...
export const cameraEnabled: Ref<boolean> = ref(readBool('mediaPref.camera', false))
export const selectedCameraId: Ref<string> = ref(readString('mediaPref.cameraId', ''))
export const selectedMicId: Ref<string> = ref(readString('mediaPref.micId', ''))
export const selectedSpeakerId: Ref<string> = ref(readString('mediaPref.speakerId', ''))
export const noiseCancellationEnabled: Ref<boolean> = ref(
  readBool('mediaPref.noiseCancellation', false)
)

export const pushToTalkEnabled: Ref<boolean> = ref(readBool('mediaPref.pushToTalk', false))

export const autoHideToolbar: Ref<boolean> = ref(readBool('mediaPref.autoHideToolbar', false))

export function setNoiseCancellationEnabled(val: boolean): void {
  noiseCancellationEnabled.value = !!val
  localStorage.setItem('mediaPref.noiseCancellation', noiseCancellationEnabled.value ? '1' : '0')
}

export function setPushToTalkEnabled(val: boolean): void {
  pushToTalkEnabled.value = !!val
  localStorage.setItem('mediaPref.pushToTalk', pushToTalkEnabled.value ? '1' : '0')
}

export function setAutoHideToolbar(val: boolean): void {
  autoHideToolbar.value = !!val
  localStorage.setItem('mediaPref.autoHideToolbar', autoHideToolbar.value ? '1' : '0')
}
...
export function loadMediaPreferences(): void {
  ...
  noiseCancellationEnabled.value = readBool('mediaPref.noiseCancellation', false)
