...
describe('sendRequest', () => {
  it('rejects when not connected', async () => {
    ...
    await expect(client.sendRequest('test', {})).rejects.toThrow('Not connected to SFU')
  })
  ...
  it('rejects when response.success is false', async () => {
    const client = createClient()
    client.connected = true
    client.signalChannel.emit = vi.fn((_event, _data, cb) => cb({ success: false, error: 'nope' }))
...
describe('sendChatMessage', () => {
  it('throws when not connected', () => {
    const client = createClient()
    client.connected = false
    expect(() => client.sendChatMessage('hello')).toThrow('Not connected to SFU')
  })
...
describe('sendRaiseHand', () => {
  ...
  it('resolves when response.success is true', async () => {
    const client = createClient()
    client.connected = true
    client.signalChannel.emit = vi.fn((_event, _data, cb) => cb({ success: true }))
...
describe('on / off event handling', () => {
  it('registers handler via on()', () => {
    ...
    expect(client.signalChannel.on).toHaveBeenCalledWith('participant_joined', handler)
  })
  ...
  it('removes handler via off()', () => {
    ...
    expect(client.signalChannel.off).toHaveBeenCalledWith('participant_joined', handler)
  })
})
