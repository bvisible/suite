...
describe('tokenizeChatMessage', () => {
  it('plain text produces a single text token', () => {
    expect(tokenizeChatMessage('hello world')).toEqual([{ type: 'text', text: 'hello world' }])
  })
  ...
  it('data: scheme URL is treated as text', () => {
    const tokens = tokenizeChatMessage('data:text/html,<script>alert(1)</script>')
    expect(tokens).toEqual([{ type: 'text', text: 'data:text/html,<script>alert(1)</script>' }])
    expect(tokens.some(t => t.type === 'link')).toBe(false)
  })

  it('mailto: and other non-http(s) schemes are treated as text', () => {
    const inputs = ['mailto:[email protected]', 'tel:+15555550100', 'ftp://x.example/']
  ...
  it('whitespace-only produces a single text token', () => {
    expect(tokenizeChatMessage('   \t  ')).toEqual([{ type: 'text', text: '   \t  ' }])
  })
