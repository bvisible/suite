...
export class SFUClient {
  ...
  async connect(meetingId: string, guestAuthToken: string | null = null): Promise<boolean> {
    if (this.connected) {
      const connectionDetails = await this.getConnectionDetails(meetingId, guestAuthToken)
  ...
  getSFUEndpoint(): { origin: string; socketPath: string } {
    ...
    const origin = isSecured ? urlObj.origin : `${urlObj.protocol}//${urlObj.hostname}:${sfuPort}`
  ...
  async refreshToken(options: { skipServerUpdate?: boolean } = {}): Promise<string> {
    ...
    try {
      ...
      this.connectionDetails.tokenExpiresAt = Date.now() + expiresInSeconds * 1000
  ...
  async createWebRtcTransport(direction: string): Promise<{
    ...
    try {
      ...
      console.warn('Failed to deep-clone transport response, returning raw response', err)
      const { id, iceParameters, iceCandidates, dtlsParameters } = response
      return { id, iceParameters, iceCandidates, dtlsParameters }
    }
  }

  async connectWebRtcTransport(transportId: string, dtlsParameters: unknown): Promise<void> {
  ...
  async getExistingProducers(roomId: string | null = null): Promise<unknown[]> {
    ...
    const response = (await this.sendRequest('get_existing_producers', requestData)) as Record<
      string,
      unknown
    >
  ...
  async joinRoom(roomId: string, userData: unknown, mediaState: unknown): Promise<unknown> {
  ...
  sendRaiseHand(raised: boolean): Promise<unknown> {
    ...
    return new Promise((resolve, reject) => {
      this.signalChannel.emit('raise_hand', { raised }, (response: Record<string, unknown>) => {
        if (response?.success) {
          resolve(response)
        } else {
          reject(new Error((response?.error as string) || 'Failed to raise hand'))
        }
      })
    })
  }
