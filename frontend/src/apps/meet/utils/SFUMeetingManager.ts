...
import { SFUConnectionManager, type SFUEventHandlers } from './sfu/SFUConnectionManager'
...
export class SFUMeetingManager {
  ...
  initialize(options: SFUMeetingManagerOptions): void {
    this.connectionManager.initialize(options.meetingId, options.currentUser, options.eventHandlers)
  }
        ...
        if (hadVideo && videoStream) {
          ...
          if (videoTrack) {
            try {
              ...
              console.warn('Failed to re-publish video after E2EE conversion:', error)
            }
          }
        }
        ...
        if (hadAudio && audioStream) {
          ...
          if (audioTrack) {
            try {
              ...
              console.warn('Failed to re-publish audio after E2EE conversion:', error)
            }
          }
        }
      }
