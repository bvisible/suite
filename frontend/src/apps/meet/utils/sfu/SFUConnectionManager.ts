...
import type { ParticipantData, ParticipantManager } from '../media/ParticipantManager'
...
export interface SFUEventHandlers {
  ...
  onParticipantLeft?: (data: { participantId: string; participant?: unknown }) => void
  onParticipantUpdated?: (participantId: string, participant: unknown, updates: unknown) => void
...
export class SFUConnectionManager {
  ...
  initialize(meetingId: string, currentUser: unknown, eventHandlers?: SFUEventHandlers): void {
      ...
      const normalized = (participants || [])
        .map((p: Record<string, unknown>) => {
          ...
          return {
            ...
            audio_enabled: typeof info.audio_enabled === 'boolean' ? info.audio_enabled : false,
            video_enabled: typeof info.video_enabled === 'boolean' ? info.video_enabled : false,
      ...
      if (existingProducers?.length) {
        console.log(
          ...
          existingProducers.map((p: unknown) => {
            ...
            return {
              id: producer.id,
              participantId: producer.participantId || producer.user_id || producer.userId,
              kind: producer.kind,
          ...
          this.bufferedProducerEvents.push(
            ...existingProducers.map((producerInfo: unknown) => {
              ...
              return {
                ...
                participantId: (info.participantId ?? info.user_id ?? info.userId) as string,
        ...
        for (const producerInfo of existingProducers) {
          const info = producerInfo as Record<string, unknown>
          const participantId = (info.participantId ?? info.user_id ?? info.userId) as string
  ...
  async flushBufferedProducers(): Promise<void> {
    ...
    console.log(`Flushing ${this.bufferedProducerEvents.length} buffered producer events`)
  ...
  private hasConsumerForProducer(participantId: string, producerId: string): boolean {
    ...
    return existingConsumers.some(
      c =>
        !c.consumer.closed && (c.producerId === producerId || c.consumer.producerId === producerId)
    )
  }
  ...
  private setupManagerEventHandlers(): void {
    this.participantManager.setEventHandlers({
      ...
      onParticipantRemoved: (participantId: string, participant: Record<string, unknown>) => {
        this.videoManager.removeVideoElement(participantId)
        this.mediaManager.consumerManager.cleanupParticipantConsumers(participantId)
      ...
      onParticipantUpdated: (
        ...
        if (this.eventHandlers.onParticipantUpdated) {
          this.eventHandlers.onParticipantUpdated(participantId, participant, updates)
        }
      },
    })
  ...
  private setupSFUEventHandlers(): void {
    ...
    this.sfuClient.on('producer_created', async (data: unknown) => {
      ...
      if (this.initialSyncInProgress || !this.transportManager?.isDeviceLoaded?.()) {
    ...
    this.sfuClient.on('producer_closed', (data: unknown) => {
      ...
      if (pid) {
        const allForPid = this.mediaManager.consumerManager.getConsumersByParticipant(pid)
        for (const c of allForPid) {
          const producedMatch =
            (closedProducerId && c.consumer.producerId === closedProducerId) ||
            c.appData?.producerId === closedProducerId
          const isScreenLike =
            c.isScreen || c.appData?.type === 'screen' || c.consumer.appData?.type === 'screen'
          const shouldRemove = producedMatch || (isScreenLike && closedIsScreen)
          if (shouldRemove) {
    ...
    this.sfuClient.on('consumer_closed', (data: unknown) => {
      try {
        ...
        const removed = this.mediaManager.consumerManager.removeConsumer(consumerId)
        if (!removed) {
          const pid = d.participantId
          if (pid) {
            const allForPid = this.mediaManager.consumerManager.getConsumersByParticipant(pid)
            for (const c of allForPid) {
              const maybeScreen =
                ...
                (c.consumer as { appData?: { type?: string } })?.appData?.type === 'screen'
              if (maybeScreen) {
                this.mediaManager.consumerManager.removeConsumer(c.id)
              }
            }
