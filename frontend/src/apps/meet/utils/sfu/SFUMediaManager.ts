...
export class SFUMediaManager {
  ...
  constructor(options: MediaManagerOptions, getCurrentUserId: () => string | null) {
  ...
  async publishMedia(
    ...
    try {
      ...
      if (publishVideo && localStream) {
        ...
        if (videoTrack) {
          try {
            const videoProducer = await this.transportManager.createProducer(videoTrack, {
              type: 'camera',
            })
      ...
      if (publishAudio && localStream) {
        ...
        if (audioTrack) {
          try {
            const audioProducer = await this.transportManager.createProducer(audioTrack, {
              type: 'microphone',
            })
  ...
  async subscribeToProducer(
    ...
    try {
      const consumer = await this.transportManager.createConsumer(producerId, metadata)

      const enhancedConsumer = this.consumerManager.addConsumer(consumer, participantId)
  ...
  async subscribeToRemoteProducer({
    ...
    this.resubscribeAttempts.delete(this.resubscribeKey(participantId, producerId))
    return result
  }
  ...
  async attachVideoConsumer(participantId: string, consumer: ConsumerEntry): Promise<void> {
    try {
      ...
      console.error(`Failed to attach video consumer for ${participantId}:`, error)
    }
  }

  async attachAudioConsumer(participantId: string, consumer: ConsumerEntry): Promise<void> {
    try {
      const track = consumer.track as MediaStreamTrack
      const stream = new MediaStream([track])
      ...
      console.error(`Failed to attach audio consumer for ${participantId}:`, error)
    }
  }
  ...
  async handleScreenShareConsumer(consumer: ConsumerEntry): Promise<void> {
    ...
    const allCameraConsumers = allConsumers.filter(c => c.kind === 'video' && !c.isScreen)
    ...
    try {
      if (allCameraConsumers.length > 0 && !this.isScreenShareActive) {
        ...
        for (const cameraConsumer of allCameraConsumers) {
          await this.attachVideoConsumer(cameraConsumer.participantId, cameraConsumer)
        }
      }
