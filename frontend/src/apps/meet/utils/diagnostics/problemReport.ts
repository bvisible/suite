...
function getTransportStats(transportManager?: TransportManagerLike | null): TransportStats {
  return (
...
async function buildProblemReportMailto(options: BuildProblemReportMailtoOptions): Promise<string> {
  const {
    meetingId,
    networkQuality,
    localStream = null,
  ...
  const bodyLines = [
    ...
    `- Outgoing bitrate (bps): ${
      networkStats?.isValid && Number.isFinite(networkStats?.availableOutgoingBitrate)
