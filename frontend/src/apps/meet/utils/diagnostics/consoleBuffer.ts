...
function ensureStore(maxEntries: number = DEFAULT_MAX_ENTRIES): DiagnosticsStore {
...
function appendEntry(level: LogLevel, args: unknown[]): void {
  ...
  const message =
    rendered.length > MAX_LINE_CHARS ? `${rendered.slice(0, MAX_LINE_CHARS)}...` : rendered
...
export function installConsoleBuffer(options: InstallConsoleBufferOptions = {}): void {
