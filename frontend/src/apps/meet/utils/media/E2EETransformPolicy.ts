...
export interface E2EETransformPolicy {
  ...
  setupSenderTransform(sender: RTCRtpSender, senderId: number, mediaType: string): Promise<boolean>
...
export class DefaultE2EETransformPolicy implements E2EETransformPolicy {
  ...
  get transformsEnabled(): boolean {
    return Boolean(this.sfuClient?.isE2EERequired?.()) && E2EEMeeting.instance.hasMeetingContext()
  }

  get legacyInsertableStreamsEnabled(): boolean {
    return this.transformsEnabled && this.sfuClient?.getE2EEMode?.() === 'insertable-streams'
  }
  ...
  assertContextReady(operation: string): void {
    if (this.sfuClient?.isE2EERequired?.() && !E2EEMeeting.instance.hasMeetingContext()) {
      throw new Error(`Cannot ${operation}: E2EE is required but meeting context is not ready`)
    }
  }
  ...
  setupSenderTransform(
    ...
    return E2EEMeeting.instance.setupSenderTransform(sender, senderId, mediaType)
  }
  ...
  setupReceiverTransform(
    ...
    return E2EEMeeting.instance.setupReceiverTransform(receiver, senderId, mediaType)
  }
}
