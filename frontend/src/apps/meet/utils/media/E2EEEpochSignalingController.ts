...
import { getActiveEpochState, installActiveEpochState } from './E2EEEpochStateStore'
import { E2EEMeeting } from './E2EEMeeting'
import { type EpochProtocolProvider, TsMlsEpochProtocolProvider } from './EpochProtocolProvider'
...
type PendingKeyPackage = Awaited<ReturnType<EpochProtocolProvider['generateKeyPackage']>>
...
export class E2EEEpochSignalingController {
  ...
  private readonly pendingKeyPackagesByEpoch = new Map<number, PendingKeyPackage>()
  ...
  constructor(deps: E2EEEpochSignalingControllerDeps) {
    this.deps = deps
    this.epochProtocolProvider = deps.epochProtocolProvider ?? new TsMlsEpochProtocolProvider()
  }
  ...
  private dispatchJoinStatus(data: Extract<E2eeEpochEnvelope, { type: 'join-status' }>): void {
  ...
  private async publishKeyPackage(
    ...
    const senderId = this.deps.sfuClient.getOwnSenderId()
    if (senderId === null) {
      console.warn('[DEBUG-e2ee] publishKeyPackage aborted: ownSenderId is null', {
        epochNumber,
      })
      return
    }
  ...
  private async authorAddMemberCommit(
    ...
    if (!activeEpoch || activeEpoch.epochNumber !== request.epochNumber) {
      console.warn('[DEBUG-e2ee] authorAddMemberCommit: epoch mismatch, abort', {
        activeEpochNumber: activeEpoch?.epochNumber ?? null,
        requestEpochNumber: request.epochNumber,
      })
      return
    ...
    if (
      ...
      console.warn('[DEBUG-e2ee] authorAddMemberCommit: missing key packages for named joiners', {
        requested: request.joiningSenderIds,
        missing: request.joiningSenderIds.filter(id => !this.receivedKeyPackagesBySenderId.has(id)),
      })
      return
    }

    const decodedKeyPackages = joiningPackages.map(p =>
      this.epochProtocolProvider.decodeKeyPackage(bytesFromBase64(p.keyPackage))
    )
    ...
    this.deps.sfuClient.sendE2EEEpochEnvelope({
      ...
      mlsCommit: bufferToBase64(this.epochProtocolProvider.encodeCommit(result.commit)),
    })
    for (const joiner of joiningPackages) {
      this.deps.sfuClient.sendE2EEEpochEnvelope({
        ...
        epochNumber: result.epoch.epochNumber,
        mlsWelcome: bufferToBase64(this.epochProtocolProvider.encodeWelcome(result.welcome)),
      })
    }
  }
  ...
  private async authorRemoveCommit(
    ...
    for (let leafIndex = 0; leafIndex * 2 + 1 < tree.length; leafIndex += 1) {
      ...
      if (!node) continue
      const credential = getCredentialFromLeafIndex(tree as never, leafIndex as never)
      if (credential.credentialType !== 'basic') continue
      const identity = this.parseCredentialIdentity(new TextDecoder().decode(credential.identity))
    ...
    for (const removedId of removedIds) {
      ...
      const result = await this.epochProtocolProvider.removeMember(activeEpoch.state, leafIndex)
      const identity = await this.deps.getDeviceIdentity()
      installActiveEpochState({
        epochNumber: result.epoch.epochNumber,
        state: result.epoch.state,
  ...
  private async processCommit(
    ...
    if (!activeEpoch || activeEpoch.epochNumber !== commitEnvelope.previousEpochNumber) {
      if (
        activeEpoch?.epochNumber === commitEnvelope.epochNumber &&
        this.deps.sfuClient.getOwnSenderId() === commitEnvelope.fromSenderId
      ) {
        console.log('[DEBUG-e2ee] processCommit: ignoring reflected self-authored commit')
        return
      }
    ...
    const [decodedCommit] = decodeMlsMessage(bytesFromBase64(commitEnvelope.mlsCommit), 0)
  ...
  private async processWelcome(
    ...
    console.log('[DEBUG-e2ee] processWelcome: enter', {
      ...
      pendingKeyPackagesByEpoch: Array.from(this.pendingKeyPackagesByEpoch.keys()),
    })
    ...
    const pendingKeyPackage = this.pendingKeyPackagesByEpoch.get(welcomeEnvelope.epochNumber - 1)
  ...
  async syncSenderSigningPubs(state: import('ts-mls').ClientState): Promise<void> {
    ...
    switch (envelope.type) {
      ...
      case 'resync-request':
        return (
          this.isBoundedString(envelope.fromParticipantId) &&
          this.isSenderId(envelope.fromSenderId) &&
          (envelope.knownEpochNumber === undefined || this.isEpochNumber(envelope.knownEpochNumber))
        )
      case 'join-status':
        return (
  ...
  private isSenderId(value: unknown): value is number {
    return (
      typeof value === 'number' && Number.isInteger(value) && value >= 0 && value <= SENDER_ID_MAX
    )
  }
