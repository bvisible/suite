...
class SendState {
  ...
  updateContext(meetingSecret: Uint8Array<ArrayBuffer>, signingPrivateKey: CryptoKey): void {
  ...
  async encrypt(frame: EncodedFrame, keyVersion: number): Promise<EncodedFrame | null> {
    const subtle = getSubtle()
    const generation = this.nextGeneration++
    const clearPrefix = getClearPrefix(frame.data, getClearPrefixSize(this.mediaType))
    const key = await deriveFrameKey(this.meetingSecret, this.senderId, this.mediaType, generation)
    ...
    const signed = buildSignedFramePayload(header, clearPrefix, FRAME_MAGIC, ciphertext)
    ...
    out.set(signature, clearPrefix.byteLength + FRAME_MAGIC.byteLength + header.byteLength)
    out.set(
      ciphertext,
      clearPrefix.byteLength + FRAME_MAGIC.byteLength + header.byteLength + signature.byteLength
    )
...
class RecvState {
  ...
  private frameKeyCache = new Map<string, { generation: number; key: CryptoKey }>()
  ...
  async warmSigningPub(pub: CryptoKey): Promise<void> {
    try {
      await getSubtle().verify({ name: 'Ed25519' }, pub, DUMMY_VERIFY_SIG, DUMMY_VERIFY_MSG)
    } catch {
  ...
  async prewarmFrameKeys(mediaType: string): Promise<void> {
    ...
    await Promise.all(
      senderIds.map(async senderId => {
        const key = await deriveFrameKey(this.meetingSecret, senderId, mediaType, 0)
  ...
  async decrypt(
    ...
    const frameMagic = data.slice(magicOffset, magicOffset + FRAME_MAGIC.byteLength)
    ...
    const signed = buildSignedFramePayload(headerFixed, clearPrefix, frameMagic, ciphertext)
    const sigOk = await subtle.verify({ name: 'Ed25519' }, signingPub, signature, signed)
    if (!sigOk || !this.markSeen(header.senderId, mediaType, header.generation)) {
      return null
    }
    const cached = this.getCachedFrameKey(header.senderId, mediaType, header.generation)
    const key =
      cached ??
      (await deriveFrameKey(this.meetingSecret, header.senderId, mediaType, header.generation))
    if (!cached) {
  ...
  private async deriveNextFrameKey(
    ...
    try {
      ...
      const nextKey = await deriveFrameKey(this.meetingSecret, senderId, mediaType, nextGeneration)
  ...
  private markSeen(senderId: number, mediaType: string, generation: number): boolean {
...
async function applyPendingPrewarm(state: RecvState): Promise<void> {
  ...
  await Promise.all(senderSigningPubs.map(([, pub]) => state.warmSigningPub(pub)))
...
self.addEventListener('message', (event: MessageEvent<WorkerMessage>) => {
  if (event.data.type === 'addSenderSigningPub') {
    ...
    if (recvState) {
      ...
      pendingRecvPrewarm = {
        ...
        senderSigningPubs: [...existing, [event.data.senderId, event.data.signingPub]],
      }
    }
    return
  }
  if (event.data.type === 'prewarm') {
    ...
    if (recvState) {
      void (async () => {
        await Promise.all(
          incoming.senderSigningPubs.map(([, pub]) => recvState?.warmSigningPub(pub))
        )
  ...
  if (event.data.type === 'updateContext') {
    ...
    if (sendState && event.data.senderSigningPrivateKey) {
      sendState.updateContext(event.data.meetingSecret, event.data.senderSigningPrivateKey)
    }
    if (recvState) {
      recvState.updateContext(event.data.meetingSecret, event.data.senderSigningPubs)
    } else {
      const merged = new Map<number, CryptoKey>()
      for (const [id, pub] of pendingRecvPrewarm?.senderSigningPubs ?? []) {
        merged.set(id, pub)
...
self.addEventListener('rtctransform', (event: Event) => {
  ...
  activeSenderSigningPubs = activeSenderSigningPubs ?? options.senderSigningPubs ?? []
  ...
  const transform = new TransformStream<EncodedFrame, EncodedFrame>({
    async transform(frame, controller) {
      if (options.direction === 'send') {
        ...
        const encrypted = await sendState.encrypt(frame, activeKeyVersion ?? options.keyVersion)
      ...
      if (!recvState) {
        recvState = new RecvState(activeMeetingSecret, activeSenderSigningPubs ?? [])
      }
