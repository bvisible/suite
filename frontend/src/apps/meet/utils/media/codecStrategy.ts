...
function detectBrowserSupport() {
  ...
  if (cache.support && nowMs() - cache.cachedAt < MAX_CACHE_MS) return cache.support
  ...
  try {
    ...
    if (vp9Codec) {
      ...
      if (Array.isArray(vp9Codec.scalabilityModes)) {
        ...
        if (result.availableScalabilityModes.length > 0) result.supportsSVC = true
      }
    }
    ...
    if (!result.supportsSVC && result.supportsVP9) {
      ...
      const isChromiumLike = /Chrome\//.test(ua) || /Chromium/.test(ua) || /Edg\//.test(ua)
...
function routerSupportsVP9(deviceCapabilities: DeviceCapabilities): boolean {
  const codecs = Array.isArray(deviceCapabilities?.codecs) ? deviceCapabilities.codecs : []
...
function routerAvailableScalabilityModes(routerCapabilities: RouterCapabilities): string[] {
  const codecs = Array.isArray(routerCapabilities?.codecs) ? routerCapabilities.codecs : []
...
export function resolveCodecStrategy({
  ...
  const routerAdvertisesModes = Array.isArray(routerModes) && routerModes.length > 0

  const canUseSVC = support.supportsSVC && routerSupportsVP9(deviceCapabilities || null)
