...
interface ParticipantEventHandlers {
  ...
  onParticipantRemoved?: (participantId: string, participant: Participant) => void
...
export class ParticipantManager {
  ...
  addParticipant(participantData: ParticipantData): Participant {
    const displayName = participantData.userData?.name || participantData.user_name || ''
    const participant: Participant = {
      user_id: participantData.participantId || (participantData.user_id as string),
      user_name: displayName,
      avatar: participantData.userData?.avatar || participantData.avatar || null,
      initials: this.generateInitials(displayName || participantData.participantId || ''),
  ...
  updateParticipant(participantId: string, updates: Record<string, unknown>): Participant | null {
    const participant = this.participants.get(participantId)
    if (participant) {
      ...
      if (this.eventHandlers.onParticipantUpdated) {
        this.eventHandlers.onParticipantUpdated(participantId, updatedParticipant, updates)
      }
  ...
  syncParticipants(serverParticipants: ParticipantData[] = []): void {
    ...
    for (const serverParticipant of serverParticipants) {
      const participantId = serverParticipant.participantId || serverParticipant.user_id || ''
