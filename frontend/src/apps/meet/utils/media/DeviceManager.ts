...
class DeviceManager {
  ...
  async detectDeviceChanges(): Promise<{
    ...
    const newCameraCount = devices.filter(d => d.kind === 'videoinput').length
    const newMicCount = devices.filter(d => d.kind === 'audioinput').length
    const newSpeakerCount = devices.filter(d => d.kind === 'audiooutput').length

    return {
      videoChanged: newCameraCount !== oldCameraCount,
      audioChanged: newMicCount !== oldMicCount || newSpeakerCount !== oldSpeakerCount,
    }
  }

  async enumerateDevices(options: { video?: boolean; audio?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      if (options.video || options.audio) {
        try {
          ...
          console.warn('Could not get media permissions for device enumeration:', error)
        }
      }
  ...
  findDeviceById(deviceId: string, deviceType: DeviceType): MediaDevice | undefined {
    const devices =
      deviceType === 'camera'
        ? this.cameras
