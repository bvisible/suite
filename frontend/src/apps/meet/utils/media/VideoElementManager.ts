...
export class VideoElementManager {
  ...
  registerVideoElement(participantId: string, element: HTMLElement): void {
    ...
    const previousVideoStream = previousElement?.srcObject as MediaStream | null
    ...
    if (previousVideoStream && !videoEl.srcObject) {
      ...
      if (videoTracks.length > 0) {
        ...
        const existingVideoTrack = (videoEl.srcObject as MediaStream)?.getVideoTracks?.()?.[0]
        const videoTrackChanged =
          !existingVideoTrack || existingVideoTrack.id !== previousVideoTrack.id
    ...
    if (this.deferredAttachments.has(participantId)) {
      const { stream, isLocal } = this.deferredAttachments.get(participantId) as DeferredAttachment
      this.attachStream(participantId, stream, isLocal)
      this.deferredAttachments.delete(participantId)
    }
  }

  async attachStream(participantId: string, stream: MediaStream, isLocal = false): Promise<void> {
    ...
    if (videoElement) {
      ...
      if (videoTracks.length > 0) {
        ...
        const isStale = lastAttach !== undefined && Date.now() - lastAttach > STALE_REATTACH_MS
        const videoTrackChanged = !existingVideoTrack || existingVideoTrack.id !== newVideoTrack.id
        ...
        if (!videoElement.srcObject || videoTrackChanged || isStale) {
          ...
          console.log(`Skipping video re-attach for ${participantId} - same track`)
        }
      }
    }
  }

  attachAudioStream(participantId: string, audioTracks: MediaStreamTrack[]): void {
    ...
    if (!audioElement) {
      ...
      if (selectedSpeakerId.value) {
        ;(
          ...
          .catch((err: Error) => {
            console.warn(`Failed to set initial speaker for ${participantId}:`, err)
          })
      }
    ...
    const isStale = lastAttach !== undefined && Date.now() - lastAttach > STALE_REATTACH_MS
    const audioTrackChanged = !existingAudioTrack || existingAudioTrack.id !== newAudioTrack.id
    ...
    if (!audioElement.srcObject || audioTrackChanged || isStale) {
      ...
      audioElement.play().catch((err: Error) => {
        console.warn(`Audio autoplay failed for ${participantId}:`, err.message)
      })
    }
  }

  async playVideo(element: HTMLVideoElement, participantId: string): Promise<boolean> {
    try {
      await element.play()
      return true
    } catch (error) {
      if ((error as DOMException).name === 'NotAllowedError') {
        console.warn(`Autoplay blocked for ${participantId}, will play on user interaction`)
        this.addUserInteractionHandler(element, participantId)
      } else {
        console.warn(`Video play failed for ${participantId}:`, (error as Error).message)
      }
      return false
    }
  }

  addUserInteractionHandler(element: HTMLVideoElement, participantId: string): void {
    const playOnInteraction = async () => {
      try {
        ...
        console.warn(`Unable to play video for ${participantId}:`, (error as Error).message)
      }
    }
