...
interface ConsumerStats {
  ...
  audio: number
  screenShare: number
  byParticipant: Record<string, { video: number; audio: number; screen: number }>
}
...
export class ConsumerManager {
  ...
  addConsumer(
    ...
    const entry: ConsumerEntry = {
      id: consumer.id,
      participantId: participantIdOverride || (consumer.appData?.userId as string) || '',
  ...
  getConsumersByParticipant(participantId: string): ConsumerEntry[] {
    return this.getAllConsumers().filter(consumer => consumer.participantId === participantId)
  }
  ...
  getVideoConsumer(participantId: string): ConsumerEntry | undefined {
    return this.getAllConsumers().find(
      consumer =>
        consumer.participantId === participantId && consumer.kind === 'video' && !consumer.isScreen
    )
  }

  getAudioConsumer(participantId: string): ConsumerEntry | undefined {
    return this.getAllConsumers().find(
      consumer => consumer.participantId === participantId && consumer.kind === 'audio'
    )
  }
  ...
  async pauseParticipantConsumers(
    ...
    const filteredConsumers = kind ? consumers.filter(c => c.kind === kind) : consumers

    const results = await Promise.all(
      filteredConsumers.map(consumer => this.pauseConsumer(consumer.id))
    )

    console.log(`Paused ${filteredConsumers.length} consumers for ${participantId}`)
    return results
  }
  ...
  async resumeParticipantConsumers(
    ...
    console.log(`Resumed ${filteredConsumers.length} consumers for ${participantId}`)
    return results
  }

  updateConsumer(consumerId: string, updates: Record<string, unknown>): ConsumerEntry | null {
    const consumer = this.consumers.get(consumerId)
    if (consumer) {
      ...
      if (this.eventHandlers.onConsumerUpdated) {
        this.eventHandlers.onConsumerUpdated(consumerId, updatedConsumer, updates)
      }
  ...
  > {
    const stats: Record<string, { video: number; audio: number; screen: number }> = {}
