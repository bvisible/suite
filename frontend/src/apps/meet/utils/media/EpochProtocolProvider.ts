...
export interface EpochProtocolProvider {
  ...
  addMember(state: ClientState, joiningMember: KeyPackage): Promise<AddMemberResult>
  addMultipleMembers(
    state: ClientState,
    joiningMembers: KeyPackage[]
  ): Promise<AddMultipleMembersResult>
  removeMember(state: ClientState, removedLeafIndex: number): Promise<RemoveMemberResult>
  joinFromWelcome(
    welcome: Welcome,
  ...
  processCommit(state: ClientState, commit: MLSMessage): Promise<EpochStateResult>
...
export class TsMlsEpochProtocolProvider implements EpochProtocolProvider {
  ...
  async createGenesisEpochWithMembers(
    ...
    if (joiningMembers.length === 0) {
      throw new Error('createGenesisEpochWithMembers requires at least one joining member')
    }
    const cipherSuite = await this.getCipherSuite()
    ...
    if (!commit.welcome) {
      throw new Error('createGenesisEpochWithMembers did not produce a welcome')
    }
  ...
  async addMember(state: ClientState, joiningMember: KeyPackage): Promise<AddMemberResult> {
  ...
  async addMultipleMembers(
    ...
    if (joiningMembers.length === 0) {
      throw new Error('addMultipleMembers requires at least one joining member')
    }
    const cipherSuite = await this.getCipherSuite()
  ...
  async removeMember(state: ClientState, removedLeafIndex: number): Promise<RemoveMemberResult> {
  ...
  async processCommit(state: ClientState, commit: MLSMessage): Promise<EpochStateResult> {
  ...
  async exportMeetingSecret(state: ClientState): Promise<Uint8Array<ArrayBuffer>> {
    const cipherSuite = await this.getCipherSuite()
  ...
  private async buildStateResult(state: ClientState): Promise<EpochStateResult> {
    return {
