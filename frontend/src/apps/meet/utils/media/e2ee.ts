...
type E2EETransformCapability = 'legacy-insertable-streams' | 'rtp-script-transform' | 'none'
...
export async function importEd25519PublicKey(b64: string): Promise<CryptoKey> {
  return getSubtle().importKey('raw', bytesFromBase64(b64), { name: 'Ed25519' }, true, ['verify'])
}
...
export async function verifyProof(
  ...
  return getSubtle().verify({ name: 'Ed25519' }, publicKey, bytesFromBase64(signatureB64), payload)
}
...
export async function generateMeetingSecret(): Promise<Uint8Array<ArrayBuffer>> {
...
export class SenderChainState {
  ...
  async signFramePayload(
    ...
    const frameMagic = maybeCiphertext ? frameMagicOrCiphertext : EMPTY_FRAME_MAGIC
...
export class ReceiverChainState {
  ...
  private readonly frameKeyCache = new Map<string, { generation: number; key: CryptoKey }>()
  ...
  async getKeyForFrame(
    ...
    if (cached && cached.generation === generation) {
      ...
      aesKey = await deriveFrameKey(this.meetingSecret, senderId, mediaType, generation)
  ...
  async verifyFrameSignature(
    ...
    if (!pub) return false
    const frameMagic = maybeSignature ? frameMagicOrCiphertext : EMPTY_FRAME_MAGIC
    const ciphertext = maybeSignature ? ciphertextOrSignature : frameMagicOrCiphertext
...
export function createEncryptionTransformStream(
  ...
  return new TransformStream({
    async transform(encodedFrame, controller) {
      const currentKeyVersion = typeof keyVersion === 'function' ? keyVersion() : keyVersion
      const subtle = getSubtle()
      const typedFrame = encodedFrame as EncodedFrameLike
      const clearPrefix = getClearPrefix(typedFrame.data, getClearPrefixSize(chainState.mediaType))
      try {
        ...
        newData.set(signature, clearPrefix.byteLength + FRAME_MAGIC.byteLength + headerBuf.length)
        newData.set(
          cipherBytes,
          clearPrefix.byteLength + FRAME_MAGIC.byteLength + headerBuf.length + signature.byteLength
        )
  ...
  return new TransformStream({
    async transform(encodedFrame, controller) {
      const currentExpectedKeyVersion =
        typeof expectedKeyVersion === 'function' ? expectedKeyVersion() : expectedKeyVersion
      const subtle = getSubtle()
      ...
      const result = await chainState.getKeyForFrame(header.senderId, mediaType, header.generation)
      if ('error' in result) {
        return
      }
      try {
