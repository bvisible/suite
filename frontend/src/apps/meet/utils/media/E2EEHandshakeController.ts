...
export class E2EEHandshakeController {
  ...
  private async collectMembersAndCreateGenesisEpoch(): Promise<boolean> {
    ...
    await this.generateHostMeetingSecret()
    const collectedSenderIds = Array.from(collected.keys()).sort((a, b) => a - b)
    const decodedKeyPackages = collectedSenderIds.map(senderId => {
      const cached = collected.get(senderId)
      return this.epochProtocolProvider.decodeKeyPackage(bytesFromBase64(cached?.keyPackage ?? ''))
    })
    ...
    await this.deps.epochSignalingController?.syncSenderSigningPubs(result.epoch.state)
  ...
  private async listCurrentNonHostSenderIds(): Promise<number[]> {
    try {
      ...
      console.warn('[DEBUG-e2ee] listCurrentNonHostSenderIds: getRoomParticipants failed', error)
      return []
    }
  }
  ...
  > {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      await new Promise(resolve => setTimeout(resolve, Math.min(250, timeoutMs / 6)))
    }
    return out
  }
  ...
  setMeetingContext(
    ...
    E2EEMeeting.instance.setMeetingContext(meetingSecret, keyVersion, signingPrivateKey)
  }
  ...
  async handleMeetingE2EEEnabled(data: { meeting_id?: string }): Promise<void> {
    ...
    if (manager && !manager.mediaHandler?.videoProducer && !manager.mediaHandler?.audioProducer) {
      console.log(
  ...
  private async reconfigureMediaForE2EE(): Promise<void> {
    ...
    const videoStreamForRepublish = this.mediaState.processedStream || this.mediaState.localStream
