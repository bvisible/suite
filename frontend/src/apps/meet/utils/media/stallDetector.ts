...
export class StallDetector {
  ...
  constructor(options: StallDetectorOptions = {}) {
    ...
    this.audioStallTimeoutMs = options.audioStallTimeoutMs ?? DEFAULT_AUDIO_STALL_TIMEOUT_MS
    this.minConsumerAgeMs = options.minConsumerAgeMs ?? DEFAULT_MIN_CONSUMER_AGE_MS
  ...
  private timeoutFor(kind: string | undefined, hasReceivedBytes: boolean): number {
    return kind === 'audio' && hasReceivedBytes ? this.audioStallTimeoutMs : this.stallTimeoutMs
  }
  ...
  private shouldRecover(st: ConsumerState, now: number): boolean {
    ...
    if (st.lastRecoveredAt !== null && now - st.lastRecoveredAt < this.recoveryCooldownMs) {
      return false
    }
    return true
  }
...
export function extractInboundBytesReceived(stats: RTCStatsReport): number | null {
