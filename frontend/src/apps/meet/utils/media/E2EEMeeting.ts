...
import { notifyE2EEContextReady, resetE2EEContextReady } from './E2EEContextReady'
import {
...
export class E2EEMeeting {
  ...
  setMeetingContext(
    ...
    if (this.senderSigningPriv) {
      for (const chain of this.senderChains.values()) {
        chain.updateContext(new Uint8Array(meetingSecretArg), this.senderSigningPriv)
      }
    } else {
      for (const chain of this.senderChains.values()) {
        chain.wipe()
  ...
  async getSessionFingerprint(): Promise<string | null> {
    ...
    const prefix = new TextEncoder().encode(`meet-e2ee-session|${this.keyVersion}|`)
  ...
  hasSenderSigningPub(senderId: number): boolean {
    return this.receiverChain?.hasSenderSigningPub(senderId) ?? this.senderSigningPubs.has(senderId)
  }
  ...
  async getE2EEChatKey(): Promise<CryptoKey | null> {
    ...
    if (this.chatKeyCache && this.chatKeyCache.meetingSecretVersion === this.keyVersion) {
    ...
    const hkdfKey = await subtle.importKey('raw', ikm as BufferSource, 'HKDF', false, [
      'deriveBits',
    ])
  ...
  async getE2EEPollKey(): Promise<CryptoKey | null> {
    if (!this.meetingSecret) return null
    if (this.pollKeyCache && this.pollKeyCache.meetingSecretVersion === this.keyVersion) {
  ...
  private getOrCreateSenderChain(senderId: number, mediaType: string): SenderChainState | null {
    if (!this.meetingSecret) {
      return null
    }
    if (!this.senderSigningPriv) {
      console.warn('[E2EE] getOrCreateSenderChain: no sender signing key (deferring)')
      return null
    }
    const key = `${senderId}:${mediaType}`
    let chain = this.senderChains.get(key)
    if (!chain) {
      chain = new SenderChainState(this.meetingSecret, senderId, mediaType, this.senderSigningPriv)
  ...
  preCreateReceiverStreams(receiver: RTCRtpReceiver): boolean {
    ...
    if (capability === 'none') {
      console.warn('[E2EE] preCreateReceiverStreams: no insertable stream support')
      return false
    }
    ...
    const streams = (receiver as ReceiverWithInsertableStreams).createEncodedStreams?.()
    if (!streams) {
      console.warn('[E2EE] preCreateReceiverStreams: createEncodedStreams returned null')
      return false
    }
  ...
  async setupSenderTransform(
    ...
    const capability = getE2EETransformCapability()
    if (this.activeSenderTransforms.has(sender) && capability !== 'rtp-script-transform') {
      return false
    }
    if (capability === 'none') {
      console.warn('[E2EE] setupSenderTransform: insertable stream support missing')
      return false
    }
    if (!this.hasMeetingContext()) {
      console.warn('[E2EE] setupSenderTransform: no meeting context (deferring)', {
        senderId,
        mediaType,
      })
    ...
    if (capability === 'rtp-script-transform') {
      ...
      const worker = this.installScriptTransform(sender as TransformableSender, {
        direction: 'send',
        meetingSecret: new Uint8Array(this.meetingSecret),
    ...
    const streams = (sender as SenderWithInsertableStreams).createEncodedStreams?.()
    if (!streams) {
      console.warn('[E2EE] setupSenderTransform: createEncodedStreams returned nothing', {
        senderId,
        mediaType,
        hasProto: typeof (sender as SenderWithInsertableStreams).createEncodedStreams,
      })
      return false
    }
    const readable = streams.readable || streams.readableStream
    ...
    try {
      readable
        .pipeThrough(
          createEncryptionTransformStream(chain, () => this.keyVersion ?? senderKeyVersion)
        )
        .pipeTo(writable)
        .catch((error: unknown) => {
  ...
  async setupReceiverTransform(
    ...
    if (capability === 'rtp-script-transform') {
      ...
      const worker = this.installScriptTransform(receiver as TransformableReceiver, {
        direction: 'recv',
        meetingSecret: new Uint8Array(this.meetingSecret),
    ...
    if (preCreated) {
      ...
      const streams = (receiver as ReceiverWithInsertableStreams).createEncodedStreams?.()
      if (!streams) {
        console.warn('[E2EE] setupReceiverTransform: createEncodedStreams returned null', {
          senderId,
          mediaType,
        })
        return false
  ...
  private createE2EEWorker(): Worker {
    const worker = new Worker(new URL('./e2eeTransformWorker.ts', import.meta.url), {
      type: 'module',
    })
