...
describe('extractInboundBytesReceived', () => {
  it('returns the inbound-rtp bytesReceived', () => {
    ...
    expect(extractInboundBytesReceived(stats as unknown as RTCStatsReport)).toBe(12345)
  })
  ...
  it('returns null when no inbound-rtp report is present', () => {
    ...
    expect(extractInboundBytesReceived(stats as unknown as RTCStatsReport)).toBeNull()
  })
})
