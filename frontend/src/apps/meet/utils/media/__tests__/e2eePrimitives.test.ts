...
describe('e2eePrimitives: HKDF info-string domain separators', () => {
  ...
  it('INFO_ENVELOPE encodes meetingId and keyVersion', () => {
    expect(INFO_ENVELOPE('vscl-sabe-ykvp', 1)).toBe('meet-e2ee|envelope|vscl-sabe-ykvp|1')
  })

  it('INFO_ENVELOPE_CONTEXT uses a leading pipe (signed-data suffix, not HKDF info)', () => {
    expect(INFO_ENVELOPE_CONTEXT('vscl-sabe-ykvp', 1)).toBe('|vscl-sabe-ykvp|1')
    expect(INFO_ENVELOPE_CONTEXT('vscl-sabe-ykvp', 1)).not.toContain('meet-e2ee')
  })
