...
describe('updateParticipant', () => {
  ...
  it('fires onParticipantUpdated event', () => {
    ...
    expect(handler).toHaveBeenCalledWith('p1', expect.objectContaining({ user_name: 'Bob' }), {
      user_name: 'Bob',
    })
  })
})
...
describe('filter methods', () => {
  it('getVideoEnabledParticipants', () => {
    const pm = createManager()
    pm.addParticipant(makeParticipantData({ user_id: 'p1', userData: { video_enabled: true } }))
    pm.addParticipant(
      makeParticipantData({
        participantId: 'p2',
        user_id: 'p2',
  ...
  it('getAudioEnabledParticipants', () => {
    const pm = createManager()
    pm.addParticipant(makeParticipantData({ user_id: 'p1', userData: { audio_enabled: true } }))
    pm.addParticipant(
      makeParticipantData({
        participantId: 'p2',
        user_id: 'p2',
...
describe('syncParticipants', () => {
  ...
  it('updates existing participants', () => {
    const pm = createManager()
    pm.addParticipant(makeParticipantData({ participantId: 'p1', user_name: 'Alice' }))
