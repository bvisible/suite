...
describe('addConsumer', () => {
  it('creates a consumer entry', () => {
    ...
    expect((entry as unknown as { participantId: string }).participantId).toBe('p1')
  ...
  it('uses participantIdOverride when provided', () => {
    ...
    assertEntry(entry)
    expect((entry as unknown as { participantId: string }).participantId).toBe('override-id')
  })
...
describe('query methods', () => {
  ...
  it('getVideoConsumer excludes screen shares', () => {
    const cm = createManager()
    cm.addConsumer(mockConsumer())
    cm.addConsumer(mockConsumer({ id: 'c3', appData: { userId: 'p1', type: 'screen' } }))
...
describe('updateConsumer', () => {
  ...
  it('fires onConsumerUpdated event', () => {
    ...
    expect(handler).toHaveBeenCalledWith('c1', expect.objectContaining({ isScreen: true }), {
      isScreen: true,
    })
  })
})
...
describe('consumer @close handling', () => {
  function setupMockConsumerWithClose(overrides: Record<string, unknown> = {}): {
  ...
  it('captures producerId on the entry', () => {
    const cm = createManager()
    const entry = cm.addConsumer(mockConsumer({ id: 'c2', producerId: 'producer-2' }))
    assertEntry(entry)
    expect((entry as unknown as { producerId: string }).producerId).toBe('producer-2')
  })
})
