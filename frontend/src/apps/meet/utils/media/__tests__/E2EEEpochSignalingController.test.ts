...
function createController(options: { isHost?: boolean } = {}) {
  ...
  const addMultipleMembers = vi.fn(async (state: unknown, joiningMembers: unknown[]) => {
  ...
  const encodeKeyPackage = vi.fn((_keyPackage: unknown) => new Uint8Array([1, 2, 3]))
...
describe('E2EEEpochSignalingController', () => {
  ...
  it('publishes an MLS key package when the SFU requests one', async () => {
    const { controller, sendE2EEEpochEnvelope, generateKeyPackage } = createController()
  ...
  it('creates a fresh genesis epoch and acknowledges it when the SFU requests one', async () => {
    wipeActiveEpochState()
    const { controller, createGenesisEpoch, sendE2EEEpochEnvelope } = createController()
  ...
  it('authors an add-member commit when this host is the designated committer', async () => {
    ...
    const { controller, sendE2EEEpochEnvelope, addMultipleMembers } = createController({
      isHost: true,
    })
  ...
  it('joins from a targeted welcome and acknowledges the installed epoch', async () => {
    const { controller, sendE2EEEpochEnvelope, joinFromWelcome } = createController()
  ...
  it('quietly ignores reflected self-authored commits after local epoch advance', async () => {
    ...
    expect(warn).not.toHaveBeenCalledWith('[DEBUG-e2ee] processCommit: epoch mismatch, abort')
    warn.mockRestore()
    wipeActiveEpochState()
  })
  ...
  it('authors a remove commit when a remove-only commit-request arrives', async () => {
    ...
    const mockTree: unknown[] = [
      {
        ...
        leaf: {
          credential: {
            ...
            identity: new TextEncoder().encode(JSON.stringify({ senderId: 7, userId: 'user-1' })),
          },
        },
      },
      undefined,
      {
        ...
        leaf: {
          credential: {
            credentialType: 'basic',
            identity: new TextEncoder().encode(JSON.stringify({ senderId: 9, userId: 'user-2' })),
          },
        },
      },
      undefined,
    ...
    const { controller, sendE2EEEpochEnvelope, removeMember } = createController({
      isHost: true,
    })
