...
describe('Crypto primitives (T1.1)', () => {
  describe('X25519 ECDH', () => {
    ...
    it('round-trips public key export/import', async () => {
      const { x25519KeyPair, exportPublicKey, importPublicKey } = await import('../e2ee')
      const kp = await x25519KeyPair()
      const b64 = await exportPublicKey(kp.publicKey)
    ...
    it('derives matching shared secrets on both sides (RFC 7748 property)', async () => {
      const { x25519KeyPair, exportPublicKey, importPublicKey, ecdhKeyAgreement } = await import(
        '../e2ee'
      )
  ...
  describe('Ed25519 signatures', () => {
    it('round-trips sign/verify', async () => {
      const { ed25519KeyPair, signProof, verifyProof } = await import('../e2ee')
    ...
    it('Ed25519 public key round-trips via export/import', async () => {
      const { ed25519KeyPair, exportEd25519PublicKey, importEd25519PublicKey } = await import(
        '../e2ee'
      )
      const kp = await ed25519KeyPair()
...
describe('Chain derivation (T1.3)', () => {
  ...
  describe('HKDF chain', () => {
    it('initSenderChain is deterministic per (meetingSecret, senderId, mediaType)', async () => {
      const { generateMeetingSecret, initSenderChain } = await import('../e2ee')
    ...
    it('initSenderChain differs across senderIds', async () => {
      ...
      expect(Buffer.from(c7).toString('hex')).not.toBe(Buffer.from(c8).toString('hex'))
    })
    ...
    it('initSenderChain differs across mediaTypes for same senderId', async () => {
      ...
      expect(Buffer.from(cVideo).toString('hex')).not.toBe(Buffer.from(cAudio).toString('hex'))
    })
    ...
    it('advanceChain produces 32-byte deterministic next tip', async () => {
      ...
      expect(Buffer.from(next1).toString('hex')).toBe(Buffer.from(next2).toString('hex'))
    ...
    it('chainTipToAESKey produces a usable AES-GCM key', async () => {
      ...
      const pt = await globalThis.crypto.subtle.decrypt({ name: 'AES-GCM', iv }, aesKey, ct)
  ...
  describe('End-to-end key derivation round-trip', () => {
    it('SenderChainState and ReceiverChainState derive matching keys for all generations', async () => {
      const { generateMeetingSecret, SenderChainState, ReceiverChainState, ed25519KeyPair } =
        await import('../e2ee')
      const meetingSecret = await generateMeetingSecret()
      const kp = await ed25519KeyPair()
      const senderState = new SenderChainState(meetingSecret, 42, 'video', kp.privateKey)
      ...
      for (let i = 0; i < 100; i++) {
        ...
        const receiverResult = await receiverState.getKeyForFrame(42, 'video', i)
    ...
    it('receiver can decrypt at any generation without advancing', async () => {
      ...
      const meetingSecret = await generateMeetingSecret()
      const kp = await ed25519KeyPair()
      const senderState = new SenderChainState(meetingSecret, 10, 'video', kp.privateKey)
      const receiverState = new ReceiverChainState(meetingSecret)
      ...
      const receiverResult = await receiverState.getKeyForFrame(10, 'video', 500)
      expect('key' in receiverResult).toBe(true)
      if ('key' in receiverResult) {
        const iv = new Uint8Array(12)
...
describe('Transform streams (T1.5 + T1.6)', () => {
  ...
  it('sender + receiver round-trip 50 frames in order', async () => {
    ...
    const decrypt = createDecryptionTransformStream(receiverState, keyVersion, undefined, 'video')
  ...
  it('legacy transform streams use the latest meeting context after epoch rotation', async () => {
    ...
    const kp = await ed25519KeyPair()
    let keyVersion = 1
    const senderState = new SenderChainState(firstSecret, 42, 'video', kp.privateKey)
  ...
  it('receiver drops frames with wrong keyVersion', async () => {
    ...
    const meetingSecret = await generateMeetingSecret()
    const kp = await ed25519KeyPair()
    const senderState = new SenderChainState(meetingSecret, 7, 'video', kp.privateKey)
    const receiverState = new ReceiverChainState(meetingSecret)
    receiverState.setSenderSigningPub(7, kp.publicKey)
    const encrypt = createEncryptionTransformStream(senderState, 1)
    const decrypt = createDecryptionTransformStream(receiverState, 2, undefined, 'video')
  ...
  it('receiver drops encrypted empty-payload frames', async () => {
    ...
    receiverState.setSenderSigningPub(7, kp.publicKey)
    const encrypt = createEncryptionTransformStream(senderState, 1)
    const decrypt = createDecryptionTransformStream(receiverState, 1, undefined, 'video')
    const decrypted = await runEncryptDecrypt(encrypt, decrypt, [makeFakeFrame(new Uint8Array())])
    expect(decrypted.length).toBe(0)
  })
  ...
  it('receiver drops frames with no signing pub registered', async () => {
    ...
    const meetingSecret = await generateMeetingSecret()
    const kp = await ed25519KeyPair()
    const senderState = new SenderChainState(meetingSecret, 5, 'video', kp.privateKey)
    const receiverState = new ReceiverChainState(meetingSecret)
    const encrypt = createEncryptionTransformStream(senderState, 1)
    const decrypt = createDecryptionTransformStream(receiverState, 1, undefined, 'video')
    const decrypted = await runEncryptDecrypt(encrypt, decrypt, [
  ...
  it('receiver drops frames signed with wrong key', async () => {
    ...
    const senderState = new SenderChainState(meetingSecret, 9, 'video', attackerKp.privateKey)
    const receiverState = new ReceiverChainState(meetingSecret)
  ...
  it('receiver drops replayed frames (exact duplicate generation)', async () => {
    ...
    const meetingSecret = await generateMeetingSecret()
    const kp = await ed25519KeyPair()
    const senderState = new SenderChainState(meetingSecret, 11, 'video', kp.privateKey)
    const receiverState = new ReceiverChainState(meetingSecret)
  ...
  it('SenderChainState.wipe() resets generation counter', async () => {
    const { generateMeetingSecret, SenderChainState, ed25519KeyPair } = await import('../e2ee')
    const meetingSecret = await generateMeetingSecret()
    const kp = await ed25519KeyPair()
    const state = new SenderChainState(meetingSecret, 1, 'video', kp.privateKey)
  ...
  it('ReceiverChainState.wipe() resets all sender chains', async () => {
    const { generateMeetingSecret, ReceiverChainState } = await import('../e2ee')
    const meetingSecret = await generateMeetingSecret()
...
describe('ReceiverChainState anti-replay behavior', () => {
  ...
  it('receiver prunes seen generations outside replay window', async () => {
    ...
    const seenFrames = (state as unknown as { seenFrames: Map<string, Set<number>> }).seenFrames
    const seen = seenFrames.get('1:video')
    expect(seen ? Array.from(seen).sort((a, b) => a - b) : []).toEqual([97, 98, 99])
...
describe('Per-sender authentication (threat model B)', () => {
  ...
  it('verifyFrameSignature returns true for a properly signed frame', async () => {
    const { generateMeetingSecret, ReceiverChainState, ed25519KeyPair, signWithEd25519 } =
      await import('../e2ee')
    const secret = await generateMeetingSecret()
    const kp = await ed25519KeyPair()
    const state = new ReceiverChainState(secret)
    ...
    const signed = new Uint8Array(header.length + clearPrefix.length + cipher.length)
    signed.set(header, 0)
    ...
    const ok = await state.verifyFrameSignature(1, header, clearPrefix, cipher, sig)
    expect(ok).toBe(true)
  })
...
describe('E2EE chain registry', () => {
  ...
  it('setMeetingContext updates active RTCRtpScriptTransform workers', async () => {
    ...
    try {
      ...
      expect(await meeting.setupSenderTransform({} as RTCRtpSender, 5, 'video')).toBe(true)
