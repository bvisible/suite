...
describe('E2EEHandshakeController', () => {
  ...
  it('installs the genesis epoch meeting secret when the host enables E2EE', async () => {
    ...
    expect(
      (controller as unknown as { sfuClient: { sendE2EEEpochEnvelope: ReturnType<typeof vi.fn> } })
        .sfuClient.sendE2EEEpochEnvelope
  ...
  it('broadcasts a key-package-request and authors a multi-joiner add commit when E2EE is enabled mid-meeting', async () => {
    ...
    const welcomeCalls = sendE2EEEpochEnvelope.mock.calls.filter(
      call =>
        Array.isArray(call) && call[0] && typeof call[0] === 'object' && call[0].type === 'welcome'
    )
    expect(welcomeCalls).toHaveLength(2)
    expect(
      welcomeCalls.some(
        call =>
          Array.isArray(call) && call[0] && typeof call[0] === 'object' && call[0].toSenderId === 9
      )
    ).toBe(true)
    expect(
      welcomeCalls.some(
        call =>
          Array.isArray(call) && call[0] && typeof call[0] === 'object' && call[0].toSenderId === 11
      )
    ).toBe(true)
