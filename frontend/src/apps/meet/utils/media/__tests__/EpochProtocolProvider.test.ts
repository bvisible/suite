...
describe('TsMlsEpochProtocolProvider', () => {
  ...
  it('adds a member and lets the joiner export the same epoch meeting secret', async () => {
    ...
    const addBob = await provider.addMember(alice.state, bobKeyPackage.publicPackage)
  ...
  it('lets existing members process add-member commits for later joiners', async () => {
    ...
    const addCarol = await provider.addMember(addBob.state, carolKeyPackage.publicPackage)
    ...
    expect([...bobAfterCarol.meetingSecret]).toEqual([...addCarol.meetingSecret])
  })
  ...
  it('removes a member and rotates the meeting secret to a new value', async () => {
    ...
    expect([...removeBob.epoch.meetingSecret]).not.toEqual([...addBob.meetingSecret])
  })
})
