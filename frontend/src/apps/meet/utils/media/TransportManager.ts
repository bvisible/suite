...
import { DefaultE2EETransformPolicy, type E2EETransformPolicy } from './E2EETransformPolicy'
import {
  audioCodecOptions,
  screenEncodings,
  svcEncodingTemplate,
...
async function applyScreenShareSenderPreferences(producer: { rtpSender?: RTCRtpSender }) {
...
export class TransportManager {
  ...
  emitTransportConnectionState(direction: Direction, state: TransportConnectionState) {
    if (typeof this.eventHandlers.onTransportConnectionStateChange === 'function') {
  ...
  private extractRouterRtpCapabilities(response: unknown): RouterCapabilities {
    if (typeof response === 'object' && response !== null && 'rtpCapabilities' in response) {
      return (response as { rtpCapabilities: RouterCapabilities }).rtpCapabilities
    }
  ...
  async initializeDevice() {
    ...
    const routerRtpCapabilities = this.extractRouterRtpCapabilities(routerCapsResp)
  ...
  async createSendTransport() {
    ...
    const shouldEnableLegacyInsertableStreams = this.e2eePolicy.legacyInsertableStreamsEnabled
  ...
  async restartTransportIce(direction: Direction) {
    ...
    const iceParameters = await this.sfuClient.restartWebRtcTransportIce(transport.id)
  ...
  async createProducer(track: MediaStreamTrack, appData: Record<string, unknown> = {}) {
    if (!this.sendTransport) await this.createSendTransport()
    if (!this.device?.canProduce?.(track?.kind || 'video')) throw new Error('Unsupported')
    if (!this.sendTransport?.produce) throw new Error('Send transport is not ready to produce')
    ...
    const setupProducerSenderTransform = async (sender: RTCRtpSender | undefined) => {
      ...
      return this.e2eePolicy.setupSenderTransform(sender, senderId, mediaType).catch(error => {
  ...
  async createConsumer(producerId: string, metadata: Record<string, unknown> = {}) {
    if (!this.device) await this.initializeDevice()
    ...
    if (!recvTransport.consume) throw new Error('Receive transport is not ready to consume')
    ...
    if (consumer) {
      ...
      if (e2eeWanted && consumer.rtpReceiver) {
        try {
          ...
          if (mediaType === 'video') {
            void this.requestConsumerKeyFrameBurst(consumer.id, rawConsumerParams.producerId)
          }
        } catch (error) {
  ...
  private async requestConsumerKeyFrameBurst(
    ...
    for (const delay of delays) {
      ...
      try {
        const result = await this.sfuClient?.requestConsumerKeyFrame?.(consumerId)
