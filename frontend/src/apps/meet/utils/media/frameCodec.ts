...
import { encodeInfo, INFO_AES, INFO_FRAME, INFO_FRAME_AT, INFO_SENDER } from './e2eePrimitives'

export const FRAME_HEADER_FIXED_SIZE = 28
const FRAME_SIGNATURE_SIZE = 64
export const FRAME_HEADER_TOTAL = FRAME_HEADER_FIXED_SIZE + FRAME_SIGNATURE_SIZE
...
export const MIN_SIGNED_ENCRYPTED_FRAME_SIZE =
  FRAME_MAGIC.byteLength + FRAME_HEADER_TOTAL + AES_GCM_TAG_SIZE + MIN_FRAME_PLAINTEXT_SIZE
...
export function encodeFrameHeader(header: E2EEFrameHeader): Uint8Array<ArrayBuffer> {
...
export function decodeFrameHeader(data: Uint8Array): E2EEFrameHeader | null {
  ...
  const view = new DataView(data.buffer, data.byteOffset, FRAME_HEADER_FIXED_SIZE)
  ...
  return {
    ...
    frameType: (encodedGeneration & FRAME_GENERATION_KEYFRAME_FLAG) !== 0 ? 'key' : 'delta',
...
export function getClearPrefix(data: ArrayBuffer, prefixSize: number): Uint8Array<ArrayBuffer> {
...
export function advanceChain(chainTip: Uint8Array<ArrayBuffer>): Promise<Uint8Array<ArrayBuffer>> {
  return hkdfBits(chainTip, encodeInfo(INFO_FRAME))
}

export function chainTipToAESKey(chainTip: Uint8Array<ArrayBuffer>): Promise<CryptoKey> {
...
export function deriveFrameKey(
  ...
  return hkdfToAESKey(meetingSecret, encodeInfo(INFO_FRAME_AT(senderId, mediaType, generation)))
}
