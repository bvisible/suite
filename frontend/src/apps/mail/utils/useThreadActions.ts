...
export function useThreadActions(deps: {
  ...
  const moveToOptions = computed(() =>
    mailboxes.data
      ?.filter(
        m =>
          ![mailbox.value, mailboxIds.sent, mailboxIds.drafts, mailboxIds.screener].includes(m.id)
      )
      .map(m => ({
        label: m._name,
  ...
  const showRemoveFrom = computed(
    () =>
      !!selections.value.length && threadMails(selections.value).some(m => m.mailboxes.length > 1)
  )
  ...
  const addToOptions = computed(() =>
    mailboxes.data
      ...
      .filter(m => {
        ...
        return !selected?.every((t: Thread) => t.mailboxes.some(mb => mb.mailbox_id === m.id))
      })
      .map(m => ({
        label: m._name,
        icon: h(Icon, { name: getIcon(m), class: FOLDER_ICON_COLOR_MAP[m.color] }),
