...
export function createFormatsEngine() {
  ...
  function _cell(id, sheet) {
    return store[sheet]?.cells?.[id] || {}
  }
  function _col(c, sheet) {
    return store[sheet]?.cols?.[c] || {}
  }
  function _row(r, sheet) {
    return store[sheet]?.rows?.[r] || {}
  }
  ...
  function getCellFormat(id, sheet = 'Sheet1') {
    return _cell(id, sheet)
  }
  ...
  function getCol(c, sheet = 'Sheet1') {
    return _col(c, sheet)
  }
  function getRow(r, sheet = 'Sheet1') {
    return _row(r, sheet)
  }
  ...
  function _shiftCells(sheet, pred, newIdFn, descending) {
    ...
    const entries = Object.entries(cells)
      ...
      .filter(({ p }) => p && pred(p))
    entries.sort((a, b) =>
      descending ? b.p.row - a.p.row || b.p.col - a.p.col : a.p.row - b.p.row || a.p.col - b.p.col
    )
