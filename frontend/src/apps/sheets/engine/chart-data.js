...
export function buildOption(config, matrix) {
  ...
  switch (config.chartType) {
    case 'pie':
      return _pieOption(config, headerRow, dataRows, encoding)
    case 'bar':
      return _cartesianOption(config, headerRow, dataRows, encoding, 'bar')
    case 'line':
      return _cartesianOption(config, headerRow, dataRows, encoding, 'line')
    case 'area':
      return _cartesianOption(config, headerRow, dataRows, encoding, 'area')
    case 'scatter':
      return _cartesianOption(config, headerRow, dataRows, encoding, 'scatter')
    default:
      return _cartesianOption(config, headerRow, dataRows, encoding, 'bar')
  }
}
...
function _cartesianOption(config, headerRow, dataRows, encoding, kind) {
  ...
  const series = seriesIx.map((colIdx, i) => ({
    ...
    symbol: kind === 'scatter' ? 'circle' : isLineish && showLabels ? 'circle' : 'none',
