...
export function tokenize(src) {
  ...
  while (i < src.length) {
    ...
    if (/[0-9]/.test(src[i]) || (src[i] === '.' && /[0-9]/.test(src[i + 1] || ''))) {
      ...
      out.push({ t: T.NUM, v: parseFloat(s) })
      continue
    }
    ...
    if (/[A-Za-z_]/.test(src[i])) {
      ...
      if (!isCall && up === 'TRUE') {
        out.push({ t: T.BOOL, v: true })
        continue
      }
      if (!isCall && up === 'FALSE') {
        out.push({ t: T.BOOL, v: false })
        continue
      }
...
function makeCriteriaTest(c) {
  if (typeof c === 'string') {
    ...
    if (c.startsWith('=')) {
      const s = c.slice(1)
      return x => String(x).toLowerCase() === s.toLowerCase()
    }
...
const FUNCTIONS = {
  ...
  MAX: args => {
    const n = flatten(args).map(toNum)
  ...
  COUNT: args =>
    flatten(args).filter(v => !isErr(v) && v !== '' && v !== null && !isNaN(Number(v))).length,
  ...
  ROUNDUP: ([v, d]) => {
    const f = Math.pow(10, toNum(d))
    return Math.ceil(toNum(v) * f) / f
  },
  ROUNDDOWN: ([v, d]) => {
    const f = Math.pow(10, toNum(d))
    return Math.floor(toNum(v) * f) / f
  },
  ...
  MOD: ([n, d]) => {
    const dn = toNum(d)
    return dn === 0 ? '#DIV/0!' : toNum(n) % dn
  },
  LOG: ([v, b]) => {
    const n = toNum(v)
    if (n <= 0) return '#NUM!'
    return Math.log(n) / Math.log(b !== undefined ? toNum(b) : 10)
  },
  LN: ([v]) => {
    const n = toNum(v)
  ...
  RAND: () => Math.random(),
  RANDBETWEEN: ([lo, hi]) =>
    Math.floor(Math.random() * (Math.floor(toNum(hi)) - Math.ceil(toNum(lo)) + 1)) +
    Math.ceil(toNum(lo)),
  FLOOR: ([v, s]) => {
    const sig = s !== undefined ? toNum(s) : 1
    return sig === 0 ? '#DIV/0!' : Math.floor(toNum(v) / sig) * sig
  },
  CEILING: ([v, s]) => {
    const sig = s !== undefined ? toNum(s) : 1
    return sig === 0 ? '#DIV/0!' : Math.ceil(toNum(v) / sig) * sig
  },
  ...
  QUOTIENT: ([n, d]) => {
    const dn = toNum(d)
    return dn === 0 ? '#DIV/0!' : Math.trunc(toNum(n) / dn)
  },
  GCD: ([a, b]) => {
    let x = Math.abs(toNum(a)),
      y = Math.abs(toNum(b))
    while (y) {
      const t = y
      y = x % y
  ...
  EVEN: ([v]) => {
    const n = Math.ceil(Math.abs(toNum(v)))
    return n % 2 === 0 ? n : n + 1
  },
  ODD: ([v]) => {
    const n = Math.ceil(Math.abs(toNum(v)))
  ...
  // Combinatorics.
  COMBIN: ([n, k]) => {
    const nn = Math.trunc(toNum(n)),
      kk = Math.trunc(toNum(k))
  ...
  SUMIF: ([range, criteria, sumRange]) => {
    const r = flatten([range]),
      s = sumRange !== undefined ? flatten([sumRange]) : r
  ...
  SUMIFS: args => {
    ...
    for (let i = 1; i < args.length - 1; i += 2) {
      const range = flatten([args[i]]),
        test = makeCriteriaTest(args[i + 1])
      range.forEach((v, j) => {
        if (!test(v)) mask[j] = false
      })
    }
  ...
  AVERAGEIF: ([range, criteria, avgRange]) => {
    const r = flatten([range]),
      a = avgRange !== undefined ? flatten([avgRange]) : r
    const test = makeCriteriaTest(criteria)
  ...
  LARGE: ([range, k]) => {
    const arr = flatten([range])
      ...
      .sort((a, b) => b - a)
    const n = toNum(k) - 1
    return n >= 0 && n < arr.length ? arr[n] : '#NUM!'
  },
  SMALL: ([range, k]) => {
    const arr = flatten([range])
      .map(toNum)
  ...
  CORREL: ([xs, ys]) => {
    const X = flatten([xs]).map(toNum),
      Y = flatten([ys]).map(toNum)
  ...
  SLOPE: ([ys, xs]) => {
    const Y = flatten([ys]).map(toNum),
      X = flatten([xs]).map(toNum)
    const n = Math.min(X.length, Y.length)
    if (n < 2) return '#DIV/0!'
    const mx = X.slice(0, n).reduce((s, v) => s + v, 0) / n
    const my = Y.slice(0, n).reduce((s, v) => s + v, 0) / n
  ...
  RIGHT: ([v, n]) => {
    const s = String(v)
    return s.substring(s.length - toNum(n))
  },
  ...
  SUBSTITUTE: ([v, old, nv, n]) => {
    ...
    return s
      .split(o)
      .reduce((acc, part, i) => (i === 0 ? part : acc + (++count === toNum(n) ? r : o) + part))
  },
  REPLACE: ([v, st, len, nv]) => {
    const s = String(v)
    return s.substring(0, toNum(st) - 1) + String(nv) + s.substring(toNum(st) - 1 + toNum(len))
  },
  FIND: ([ft, wt, st]) => {
    const idx = String(wt).indexOf(String(ft), (toNum(st) || 1) - 1)
    return idx === -1 ? '#VALUE!' : idx + 1
  },
  SEARCH: ([ft, wt, st]) => {
    const idx = String(wt)
      .toLowerCase()
      .indexOf(String(ft).toLowerCase(), (toNum(st) || 1) - 1)
    return idx === -1 ? '#VALUE!' : idx + 1
  },
  VALUE: ([v]) => {
  ...
  DATE: ([y, m, d]) => {
    const dt = new Date(toNum(y), toNum(m) - 1, toNum(d))
  ...
  VLOOKUP: ([lookup, table, colIdx, rangeLookup]) => {
    ...
    if (!exact) {
      ...
      for (const row of table) {
        ...
        if (toNum(row[0]) <= toNum(lookup)) best = row
        else break
      }
  ...
  HLOOKUP: ([lookup, table, rowIdx, rangeLookup]) => {
    ...
    if (Array.isArray(lookup)) lookup = Array.isArray(lookup[0]) ? lookup[0][0] : lookup[0]
    const ri = toNum(rowIdx) - 1,
      first = table[0]
    const exact = rangeLookup === false || rangeLookup === 0
    let ci = -1
    if (!exact) {
      for (let i = 0; i < first.length; i++) {
        if (toNum(first[i]) <= toNum(lookup)) ci = i
        else break
      }
    } else {
  ...
  MATCH: ([lookup, range, matchType]) => {
    const arr = flatten([range]),
      mt = matchType !== undefined ? toNum(matchType) : 1
    if (mt === 0) {
    ...
    if (mt === 1) {
      for (let i = 0; i < arr.length; i++) {
        if (toNum(arr[i]) > toNum(lookup)) return i > 0 ? i : '#N/A'
      }
      return arr.length
    }
    if (mt === -1) {
  ...
  INDEX: ([range, rowNum, colNum]) => {
    const r = toNum(rowNum) - 1,
      c = colNum !== undefined ? toNum(colNum) - 1 : 0
    if (!Array.isArray(range)) return range
    const row = Array.isArray(range[r]) ? range[r] : range
    return row[Array.isArray(range[r]) ? c : r] !== undefined
      ? row[Array.isArray(range[r]) ? c : r]
      : '#REF!'
  },
  CHOOSE: ([idx, ...vals]) => {
    const i = toNum(idx)
    return i >= 1 && i <= vals.length ? vals[i - 1] : '#VALUE!'
  },
  ...
  COLUMNS: ([v]) =>
    Array.isArray(v) && Array.isArray(v[0]) ? v[0].length : Array.isArray(v) ? v.length : 1,
...
function createParser(
  ...
  function comparison() {
    ...
    while (peek()?.t === T.OP && ['=', '<>', '>', '<', '>=', '<='].includes(peek().v)) {
      ...
      switch (op) {
        case '=':
          l = String(l).toLowerCase() === String(r).toLowerCase() || toNum(l) === toNum(r)
          break
        case '<>':
  ...
  function pow() {
    ...
    if (peek()?.t === T.OP && peek().v === '^') {
      next()
      const bn = toNumStrict(b)
      if (isErr(bn)) return bn
      const en = toNumStrict(unary())
  ...
  function primary() {
    ...
    if (tok.t === T.REF) {
      ...
      if (peek()?.t === T.COLON) {
        ...
        const endRef = endTok.t === T.COLREF ? `${endTok.v}${tok.v.match(/[0-9]+$/)[0]}` : endTok.v
    ...
    if (tok.t === T.COLREF) {
      ...
      if (peek()?.t === T.COLON) {
        next()
        const endTok = next()
        if (!endTok || (endTok.t !== T.COLREF && endTok.t !== T.REF))
          throw new Error('Bad col range')
    ...
    if (tok.t === T.SHEETREF) {
      ...
      if (peek()?.t === T.COLON) {
        next()
        const endTok = next()
        if (!endTok || (endTok.t !== T.REF && endTok.t !== T.SHEETREF && endTok.t !== T.COLREF))
          throw new Error('Bad sheet range')
        const endRef =
          endTok.t === T.SHEETREF
            ? endTok.v
            : endTok.t === T.COLREF
    ...
    if (tok.t === T.SHEETCOL) {
      ...
      if (peek()?.t === T.COLON) {
        ...
        const endTok = next()
        const endCol = endTok
          ? endTok.t === T.SHEETCOL || endTok.t === T.COLREF
            ? endTok.v
            : endTok.v.match(/^[A-Z]+/)?.[0] || tok.v
          : tok.v
...
export function getFunctionHint(name) {
  return FN_HINTS[name] || '(...)'
}
