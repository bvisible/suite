...
export function createSheet({ onCellChanged, onCellsChanged } = {}) {
  ...
  function setNamedRangeResolver(fn) {
    resolveNamedRange = fn || null
  }
  ...
  let _memoHits = 0,
    _memoMisses = 0 // diagnostic counters
  ...
  function _clearAllMemo() {
    for (const k of Object.keys(_memo)) _memo[k].clear()
  }

  function getCellValue(id, sheet = current) {
    const raw = sheets[sheet]?.[id]
    if (raw === undefined || raw === null || raw === '') return 0
    if (typeof raw === 'string' && raw.startsWith('=')) return _evalFormula(raw.slice(1), sheet, id)
  ...
  function getCell(id, sheet = current) {
  ...
  function getRawData(sheet = current) {
    return sheets[sheet] || {}
  }
