...
export function editCellOp({ sheet, id, before, after }) {
  return {
    ...
    cells: [{ sheet, id }],
    do(ctx) {
      ctx.sheet.setCell(id, after, sheet)
    },
    undo(ctx) {
      ctx.sheet.setCell(id, before, sheet)
    },
    serialize() {
      return { type: 'edit-cell', sheet, id, before, after }
    },
    coalesce(prev) {
...
export function editCellsOp({ sheet, edits }) {
  return {
    ...
    do(ctx) {
      for (const { id, after } of edits) ctx.sheet.setCell(id, after, sheet)
    },
    undo(ctx) {
      for (const { id, before } of edits) ctx.sheet.setCell(id, before, sheet)
    },
    serialize() {
      return { type: 'edit-cells', sheet, edits }
    },
  }
}
...
export function setFormatsOp({ sheet, changes }) {
  ...
  return {
    ...
    do(ctx) {
      for (const { id, after } of changes) _applyFmt(ctx, sheet, id, after)
    },
    undo(ctx) {
      for (const { id, before } of changes) _applyFmt(ctx, sheet, id, before)
    },
    serialize() {
      return { type: 'set-formats', sheet, changes }
    },
  }
}
...
export function setValidationOp({ sheet, id, before, after }) {
  return {
    ...
    cells: [{ sheet, id }],
    do(ctx) {
      _applyVal(ctx, sheet, id, after)
    },
    undo(ctx) {
      _applyVal(ctx, sheet, id, before)
    },
    serialize() {
      return { type: 'set-validation', sheet, id, before, after }
    },
  }
}
...
export function setCommentOp({ sheet, id, before, after }) {
  return {
    ...
    cells: [{ sheet, id }],
    do(ctx) {
      _applyComment(ctx, sheet, id, after)
    },
    undo(ctx) {
      _applyComment(ctx, sheet, id, before)
    },
    serialize() {
      return { type: 'set-comment', sheet, id, before, after }
    },
  }
}
...
export function mergeOp({ sheet, range, prevMasterValues }) {
  ...
  return {
    ...
    do(ctx) {
      ctx.merge.merge(range.r0, range.c0, range.r1, range.c1)
    },
    undo(ctx) {
      ctx.merge.unmerge(range.r0, range.c0, range.r1, range.c1)
      for (const [id, v] of Object.entries(prevMasterValues || {})) ctx.sheet.setCell(id, v, sheet)
    },
    serialize() {
      return { type: 'merge', sheet, range, prevMasterValues }
    },
  }
}
...
export function unmergeOp({ sheet, range }) {
  return {
    ...
    serialize() {
      return { type: 'unmerge', sheet, range }
    },
  }
}
...
export function resizeColOp({ col, before, after }) {
  return {
    ...
    col,
    do(ctx) {
      ctx.grid.setColWidth(col, after)
    },
    undo(ctx) {
      ctx.grid.setColWidth(col, before)
    },
    serialize() {
      return { type: 'resize-col', col, before, after }
    },
  }
}
...
export function resizeRowOp({ row, before, after }) {
  return {
    ...
    row,
    do(ctx) {
      ctx.grid.setRowHeight(row, after)
    },
    undo(ctx) {
      ctx.grid.setRowHeight(row, before)
    },
    serialize() {
      return { type: 'resize-row', row, before, after }
    },
  }
}
...
export function setFreezeOp({ before, after }) {
  ...
  return {
    type: 'set-freeze',
    do(ctx) {
      ctx.view.setFreeze(after.rows, after.cols)
    },
    undo(ctx) {
      ctx.view.setFreeze(before.rows, before.cols)
    },
    serialize() {
      return { type: 'set-freeze', before, after }
    },
  }
}
...
export function setHiddenRowsOp({ before, after }) {
  return {
    ...
    do(ctx) {
      ctx.view.setHiddenRows(new Set(after))
    },
    undo(ctx) {
      ctx.view.setHiddenRows(new Set(before))
    },
    serialize() {
      return { type: 'set-hidden-rows', before, after }
    },
  }
}
...
export function setHiddenColsOp({ before, after }) {
  return {
    ...
    do(ctx) {
      ctx.view.setHiddenCols(new Set(after))
    },
    undo(ctx) {
      ctx.view.setHiddenCols(new Set(before))
    },
    serialize() {
      return { type: 'set-hidden-cols', before, after }
    },
  }
}
...
export function engineSliceOp({ type, sheet, before, after, cells = null }) {
  ...
  return {
    ...
    cells: cells || [],
    do(ctx) {
      _restoreSlices(ctx, after)
    },
    undo(ctx) {
      _restoreSlices(ctx, before)
    },
    serialize() {
      return { type, sheet, before, after, cells }
    },
  }
}
...
function _rangeCells(sheet, { r0, c0, r1, c1 }) {
  const out = []
  for (let r = r0; r <= r1; r++) for (let c = c0; c <= c1; c++) out.push({ sheet, r, c })
  return out
}
