...
export function createDepsEngine() {
  ...
  function _key(sheet, cellId) {
    return `${sheet}|${cellId}`
  }
  function _split(key) {
    const i = key.indexOf('|')
    return { sheet: key.slice(0, i), cellId: key.slice(i + 1) }
  }
  ...
  function extractRefs(formula, currentSheet) {
    ...
    try {
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        if (tok.t === 'SHEETREF') {
          ...
          if (tokens[i + 1]?.t === 'COLON' && tokens[i + 2]) {
            ...
            const endCell =
              endTok.t === 'SHEETREF' ? endTok.v : endTok.t === 'REF' ? endTok.v : null
            if (endCell) {
      ...
      /* tokeniser explodes → no deps tracked */
    }
    return refs
  }
