...
function buildEditGroup({
  ...
  return {
    ...
    items: [
      ...
      item('find', 'Find & replace', 'Ctrl+F', () => {
        showFindReplace.value = true
      }),
      item('formulas', 'Show formulas', 'Ctrl+`', () => {
...
function buildStructureGroup({
  ...
  return {
    ...
    component: CommandPaletteItem,
    items: [
      item('row-above', 'Insert row above', '', () => {
        contextMenu.targetRow = getGrid().getSelection().r0
        doInsertRow(false)
      }),
      ...
      item('row-del', 'Delete row', '', () => {
        contextMenu.targetRow = getGrid().getSelection().r0
        doDeleteRow()
      }),
      item('col-left', 'Insert column left', '', () => {
        contextMenu.targetCol = getGrid().getSelection().c0
        doInsertCol(false)
      }),
      ...
      item('autofit-w', 'Auto-fit column width', '', () => {
        contextMenu.targetCol = getGrid().getSelection().c0
        doAutoFitCol()
      }),
      item('autofit-h', 'Auto-fit row height', '', () => {
        contextMenu.targetRow = getGrid().getSelection().r0
        doAutoFitRow()
      }),
...
function buildViewGroup({
  ...
  return {
    ...
    component: CommandPaletteItem,
    items: [
      item('freeze-row', 'Freeze rows up to selection', '', () => {
        contextMenu.targetRow = getGrid().getSelection().r0
        doFreezeRow()
      }),
      item('freeze-col', 'Freeze cols up to selection', '', () => {
        contextMenu.targetCol = getGrid().getSelection().c0
        doFreezeCol()
      }),
      item('unfreeze-row', 'Unfreeze rows', '', () => doUnfreezeRows()),
      item('unfreeze-col', 'Unfreeze columns', '', () => doUnfreezeCols()),
      item('filter', 'Toggle filter', '', () => {
...
function buildInsertGroup({ openPivotDialog }) {
  return {
    title: 'Insert',
    component: CommandPaletteItem,
    items: [item('pivot-insert', 'Insert pivot table', '', () => openPivotDialog())],
  }
}

function buildSheetGroup({
  addSheet,
  currentSheet,
  openRenameDialog,
  doDuplicateSheet,
