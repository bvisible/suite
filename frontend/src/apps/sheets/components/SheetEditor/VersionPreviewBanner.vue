  ...
  <div v-if="open" class="sn-vp-banner">
    <div class="sn-vp-left">
      ...
      <span v-if="diff" class="sn-vp-diff">
        ·
        <b>{{ diff.total_changed_cells }}</b>
        edit{{ diff.total_changed_cells === 1 ? '' : 's' }}
        <span class="sn-vp-rows-meta"
          >({{ diff.total_changed_rows }}
          row{{ diff.total_changed_rows === 1 ? '' : 's' }}
          changed)</span
        >
      </span>
    </div>
    <div class="sn-vp-mid">
      <Button
        v-if="diff && diff.total_changed_cells > 0"
        size="sm"
        variant="ghost"
        icon="chevron-up"
        ...
        @click="$emit('step', -1)"
      />
      <span v-if="diff && diff.total_changed_cells > 0 && stepIndex !== null" class="sn-vp-step">
        {{ stepIndex + 1 }}
        / {{ diff.total_changed_cells }}
      </span>
      <Button
        v-if="diff && diff.total_changed_cells > 0"
        size="sm"
    ...
    <div class="sn-vp-right">
      <Button size="sm" variant="ghost" iconLeft="edit-2" @click="$emit('name')"
        >{{ version?.version_name ? 'Rename' : 'Name version' }}</Button
      >
      <Button size="sm" variant="solid" :loading="restoring" @click="$emit('restore')"
