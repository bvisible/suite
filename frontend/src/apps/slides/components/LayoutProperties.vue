...
const initMoveToIndexAndFactor = (elements, sortedActiveElements, action) => {
  ...
  switch (action) {
    ...
    case 'backward':
      const lowerZIndices = elements.filter(el => isOverlappingElement(el)).map(el => el.zIndex)
      return {
        moveToIndex: lowerZIndices.length ? Math.max(...lowerZIndices) : 1,
        factor: 1,
      }
    case 'forward':
      const higherZIndices = elements.filter(el => isOverlappingElement(el)).map(el => el.zIndex)
      return {
