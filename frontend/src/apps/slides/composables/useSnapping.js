...
export const useSnapping = (selectionRef, currentResizer, hasOngoingInteraction) => {
  ...
  const createAlignment = (source, selectionLine, guideLine, guidePosition, selectionPosition) => ({
  ...
  const getSlideAlignmentsForDrag = selectionScreenRect => {
    ...
    const x = createAlignment('slide', 'centerX', 'centerX', center.x, selectionScreenRect.centerX)
    const y = createAlignment('slide', 'centerY', 'centerY', center.y, selectionScreenRect.centerY)
    return { x: [x], y: [y] }
  }
