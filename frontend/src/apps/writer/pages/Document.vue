<template>
  ...
  <div v-else-if="!document?.doc" class="flex-1 overflow-y-auto flex justify-center">
    <div class="w-full md:min-w-[48rem] md:max-w-[48rem] px-5 pt-10 space-y-3">
      ...
      <div class="h-3" />
      <Skeleton
        v-for="(w, i) in ['92%', '78%', '85%', '65%', '88%', '40%', '80%', '70%', '60%', '84%']"
        :key="i"
        class="h-3.5 rounded"
        :style="{ width: w }"
      />
...
import { ref, inject, defineAsyncComponent, provide, watch, computed, useTemplateRef } from 'vue'
