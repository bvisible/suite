...
//// Neoffice — the shared NeoCockpit chrome wraps Calendar's AppSidebar: same
//// left rail as the rest of Neoffice, plus the per-calendar gear (rename, colour,
//// share, CalDAV, delete) that upstream has no UI for. It relays every AppSidebar
...
//// Neoffice — toggling a calendar's visibility also PERSISTS it (isVisible on
//// the JMAP server), so the show/hide choice survives a reload; upstream keeps it
//// in client state only. Endpoint moved with b8df8ca9e (mail -> calendar module).
const setVisibility = createResource({
	url: 'suite.calendar.doctype.calendar.calendar.set_calendar_visibility',
	onError: (error) => raiseToast(error.message, 'error'),
})
